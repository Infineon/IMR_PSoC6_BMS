/*
 * IMR_CAN.c - Abstraction off all CAN receive and transmit actions. Valid incoming messages set flags that are handled and reset in main.
 *
...
/* CAN ID filter setup object (needed because the original object seems to be locked). Filter range covers first to last BMS related message ID.*/
...
void CAN_IRQ_RX_MESSAGE_PARSE(uint32_t id, uint8_t data[8], uint8_t size){
	...
	if(id == BMS_SLOT_1_HS_HANDSHAKE || id == BMS_SLOT_2_HS_HANDSHAKE){
		...
		CanRequested_SamplingTime = (uint32_t)((data[0]<<24) + (data[1]<<16) + (data[2]<<8)  + (data[3]<<0));;
