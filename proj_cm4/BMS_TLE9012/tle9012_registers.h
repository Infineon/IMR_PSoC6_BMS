...
// Settings
//****************************************************************************
// Number of nodes that shall be initialized. This must match exactly the number of TLE9012 Nodes that are to be addressed
#define TLE9012_NODES_SIZE 				1
// Defines the minimum time in ms that must pass between watchdog refreshes, used to keep the ICs alive.
// Note: Per default the ICs can go to sleep when they did not receive a update after 1.8 seconds.
//       However the here mentioned time must take the maximum time between tle9012_manageQueue() executions into account (time between main loops).
...
#define CELL_OVUV_LSB		 0.0048828125		// =(TEMP_FSR / 2^10). LSB used to set cell over and undervoltage thresholds. E.g. OL_OV_THR = 4.6/CELL_OVUV_LSB
...
// Global flags
//****************************************************************************
extern uint8_t tle9012_problemDetected;  // Used to check if an action was successful (e.g. switchON()) and if failure flags are set
...
// Representation item of each TLE9012 IC management, state, register data and converted values
typedef struct {
	//****************************************************************************
	...
	//****************************************************************************
	// Register ready/error bitmap - Every bit of these maps represents if the related register
	// index is ready and without error
	//		if command is queued - ready is set to 0 until result has been received or timeout occurred
...
extern reg_item  reg_items[NUM_REGISTERS];      // Array of register addresses and names
extern const subreg_item* subreg_items[NUM_SUBREGISTERS]; // Array of subregister addresses, masks, and names (combines all predefined subreg_items to make the iterable)
// Array of all nodes, holden all data of a node (shadow registers, bitmaps, conversion results, etc)
...
// Time (ms) of the last refresh of the watchdog counter. If the time difference exceeds WD maximum interval the device will go to sleep
...
#define FRAME_ID_READ 		0b00000000	// Template for a ID frame to read a register. Actual device ID must be added e.g. via or command
#define FRAME_ID_WRITE 		0b10000000	// Template for a ID frame to write a register. Actual device ID must be added e.g. via or command
#define FRAME_ID_BROADCAST 	0b00111111	// Template for a ID frame to execute a broadcast. Read/write bit must be added e.g. via or command
...
#define  MULTI_READ_CFG 45 				  // 0032H Multiread configuration (This register must be written with a broadcast write command)
...
//****************************************************************************
// Macro definition of subregister items and settings - These shall be used whenever a subreg or setting parameter is given
//****************************************************************************
...
extern const subreg_item  OL_OV_THR__OV_THR; 						// RW - Overvoltage fault threshold 10-bit overvoltage fault threshold. Battery input voltages (U0 to U11)are tested for overvoltage with given value. OV error is detected andindicated in GEN_DIAG register if cell voltage is higher than OV faultthreshold.
    #define OV_THR__THRESHOLD				 0b0000001111111111	// Threshold (default) 
extern const subreg_item  OL_OV_THR__OL_THR_MAX; 					// RW - Open load maximum voltage drop threshold (LSB8) 6-bit (LSB8) to define the maximum threshold for the voltage dropwhile OL-diagnostics (I OL_DIAG * R F). If voltage drop > OL_THR_MAX, theOLx bit of channel x is set.
...
extern const subreg_item  OL_UV_THR__UV_THR; 						// RW - Undervoltage fault threshold 10-bit undervoltage fault threshold. Battery input voltages (U0 to U11)are tested for undervoltage with given value. UV error is detected andindicated in GEN_DIAG register if cell voltage is lower than UV faultthreshold.
    #define UV_THR__THRESHOLD				 0b0000000000000000	// Threshold (default) 
extern const subreg_item  OL_UV_THR__OL_THR_MIN; 					// RW - Open load minimum voltage drop threshold (LSB8) 6-bit (LSB8) to define the minimum threshold for the voltage drop whileOL-diagnostics (I OL_DIAG * R F). If voltage drop < OL_THR_MIN, the OLxbit of channel x is set.
...
extern const subreg_item  TEMP_CONF__EXT_OT_THR; 					// RW - External overtemperature threshold 10-bit overtemperature fault threshold. Overtemperature error isdetected and indicated in GEN_DIAG register if the externaltemperature result is lower than the fault threshold. Balancing isdeactivated.
...
extern const subreg_item  INT_OT_WARN_CONF__INT_OT_THR; 			// RW - Internal overtemperature threshold 10-bit overtemperature fault threshold. Overtemperature error isdetected and indicated in GEN_DIAG register if the internal temperatureresult is LOWER than the fault threshold. Balancing is deactivated.
...
extern const subreg_item  RR_ERR_CNT__NR_ERR; 						// RW - Number of errors Number of consecutive detected errors before error is valid and setin GEN_DIAG and individual fault registers. Only used for faults wherecounter NR_ERR is active (this can be set in register NR_ERR_MASK).Please note: The register CRC errors as well as the internal IC errors donot have an error counter.
...
extern const subreg_item  RR_CONFIG__RR_SYNC; 						// RW - Round robin synchronization
    ...
    #define RR_SYNC__SYNC					 0b0000000010000000	// Synch with WD_CNT write access (RR will be started by write access, if RR is already running the RR will be restarted from beginning again).
...
extern const subreg_item  FAULT_MASK__ERR_PIN; 						// RW - Enable Error PIN functionality
    #define ERR_PIN__DISABLE				 0b0000000000000000	// ERR pin deactivated, EMM signal active. Device goes back to the mode as it was before the EMM (default).
    #define ERR_PIN__ENABLE					 0b0000000000100000	// ERR Pin function enabled. Fault indication only via ERR Pin. EMM signal deactivated. If ERR PIN triggered, pin stays high(device is then in normal mode) until watchdog runs out or pin iscleared.
...
extern const subreg_item  GEN_DIAG__LOCK_MEAS; 						// RH - Lock measurement This bit indicates an ongoing PCVM, SCVM, BVM or AVM measurement ora delayed RR.
...
extern const subreg_item  GEN_DIAG__RR_ACTIVE; 						// RH - Round robin active This bit indicates if the round robin was active during read.
...
extern const subreg_item  GEN_DIAG__OL_ERR; 						// ROCWL - Open load error Please note: Resetting the error here resets also the respective detailederror register.
    #define OL_ERR__NO_ERR					 0b0000000000000000	// No open load error (default) 
    #define OL_ERR__ERR_OCCURED				 0b0000000010000000	// Open load error occured. Detailed information in the respective error register. Balancing deactivated.
...
extern const subreg_item  GEN_DIAG__REG_CRC_ERR; 					// ROCWL - Register CRC error Please note: Resetting the error here resets also the respective detailederror register.
    #define REG_CRC_ERR__NO_ERR				 0b0000000000000000	// No CRC check error (default) 
    #define REG_CRC_ERR__ERR_OCCURED		 0b0000001000000000	// CRC check error occured. Detailed information in the respective error register. Balancing deactivated.
extern const subreg_item  GEN_DIAG__EXT_T_ERR; 					// ROCWL - External temperature error Please note: Resetting the error here resets also the respective detailederror register.
    #define EXT_T_ERR__NO_ERR				 0b0000000000000000	// No external temperature error (default) 
    #define EXT_T_ERR__ERR_OCCURED			 0b0000010000000000	// External temperature error occured. Detailed information in the respective error register. Balancing isdeactivated.
...
extern const subreg_item  GEN_DIAG__CELL_UV; 						// ROCWL - Cell undervoltage (UV) error Please note: Resetting the error here resets also the respective detailederror register.
...
extern const subreg_item  GEN_DIAG__CELL_OV; 						// ROCWL - Cell overvoltage (OV) error Please note: Resetting the error here resets also the respective detailederror register.
...
extern const subreg_item  GEN_DIAG__BAL_ERR_UC; 					// ROCWL - Balancing error undercurrent (will be reset in sleep mode) Please note: Resetting the error here resets also the respective detailederror register.
...
extern const subreg_item  GEN_DIAG__BAL_ERR_OC; 					// ROCWL - Balancing error overcurrent (will be reset in sleep mode) Please Note: Resetting the error here resets also the respective detailederror register.
...
extern const subreg_item  REG_CRC_ERR__ADDR; 						// ROCW - Register address of register where CRC check failed Register address of CRC check fail, can also be cleared on write '0' toconnected GEN_DIAG register bit.
...
extern const subreg_item  OP_MODE__SLEEP_REG_RESET; 				// RWH - Sleep mode registers reset
    ...
    #define SLEEP_REG_RESET__SLEEP_REG_RESET_OP 0b0000000000000100	// Resets all registers that are supplied in sleep mode. No further communication possible after bit is set to '1'.
...
extern const subreg_item  BAL_CURR_THR__OC_THR; 					// RW - Overcurrent fault threshold 8-bit to define the maximum voltage drop during balancing diagnostics. If the voltage drop (I Bal*R F) > OC_THR the overcurrentis detected.
    #define OC_THR__DEFAULT					 0b0000000010101100	// Default 
extern const subreg_item  BAL_CURR_THR__UC_THR; 					// RW - Undercurrent fault threshold 8-bit to define the minimum voltage drop during balancing diagnostics.If the voltage drop (I Bal*R F) < UC_THR the undercurrent is detected.
...
extern const subreg_item  AVM_CONFIG__AUX_BIPOLAR; 				// RWH - Bipolar AUX measurement instead of BVM
    ...
    #define AUX_BIPOLAR__AUX				 0b0000000100000000	// Bipolar AUX measurement instead of BVM (Bit is reset when NR_TEMP_SENSE > 3, this can cause a REG_CRC_ERR).
...
extern const subreg_item  EXT_TEMP_3__RESULT; 						// RH - Result of external temp measurement SD-ADC result of resistance or voltage measurement.
    #define RESULT__DEFAULT					 0b0000000000000000	// Default 
extern const subreg_item  EXT_TEMP_3__INTC; 						// RH - Indicates which current source was used Number of current source that was active for latest measurement.
    #define INTC__I_0						 0b0000000000000000	// Source ITMPz_0 used (default) 
    #define INTC__I_1						 0b0000010000000000	// Source ITMPz_1 used 
    #define INTC__I_2						 0b0000100000000000	// Source ITMPz_2 used 
    #define INTC__I_3						 0b0000110000000000	// Source ITMPz_3 used 
...
extern const subreg_item  EXT_TEMP_4__RESULT; 						// RH - Result of external temp measurement SD-ADC result of resistance or voltage measurement.
    #define RESULT__DEFAULT					 0b0000000000000000	// Default 
extern const subreg_item  EXT_TEMP_4__INTC; 						// RH - Indicates which current source was used Number of current source that was active for latest measurement.
    #define INTC__I_0						 0b0000000000000000	// Source ITMPz_0 used (default) 
    #define INTC__I_1						 0b0000010000000000	// Source ITMPz_1 used 
    #define INTC__I_2						 0b0000100000000000	// Source ITMPz_2 used 
    #define INTC__I_3						 0b0000110000000000	// Source ITMPz_3 used 
...
extern const subreg_item  EXT_TEMP_R_DIAG__RESULT; 					// RH - Result of diagnostics resistor measurement SD-ADC result of diagnostics resistor measurement.
    #define RESULT__DEFAULT					 0b0000000000000000	// Default 
extern const subreg_item  EXT_TEMP_R_DIAG__INTC; 					// RH - Indicates which current source was used Number of current source that was active for latest measurement.
    #define INTC__I_0						 0b0000000000000000	// Source ITMPz_0 used (default) 
    #define INTC__I_1						 0b0000010000000000	// Source ITMPz_1 used 
    #define INTC__I_2						 0b0000100000000000	// Source ITMPz_2 used 
    #define INTC__I_3						 0b0000110000000000	// Source ITMPz_3 used 
...
extern const subreg_item  INT_TEMP__RESULT; 						// RH - Result of internal temperatur measurement SD-ADC result of internal temperature measurement.
    #define RESULT__DEFAULT					 0b0000000000000000	// Default 
...
extern const subreg_item  MULTI_READ__RES; 							// RH - Used in combination with MULTI_READ_CFG Reading this register by the host starts the multiple register readroutine which got define in the MULTI_READ_CFG register.
...
extern const subreg_item  BAL_DIAG_OC__OC_i; 						// ROCW - Balancing overcurrent in cell i Can also be cleared on write '0' to connected GEN_DIAG register bit.
    #define OC_i__NO_OC						 0b0000000000000000	// No balancing overcurrent detected in respective cell (default) 
    #define OC_i__OC						 0b0000100000000000	// Balancing overcurrent detected in respective cell. Balancing is deactivated for this cell.

// BAL_DIAG_UC - 0034H Passive balancing diagnostics UNDERCURRENT
extern const subreg_item  BAL_DIAG_UC__UC_i; 						// ROCW - Balancing undercurrent in cell i Can also be cleared on write '0' to connected GEN_DIAG register bit.
    #define UC_i__NO_UC						 0b0000000000000000	// No balancing undercurrent detected in respective cell (default)
    #define UC_i__UC						 0b0000100000000000	// Balancing undercurrent detected in respective cell. Balancing is deactivated for this cell.

// INT_TEMP_2 - 0035H Chip temperature 2
extern const subreg_item  INT_TEMP_2__RESULT; 						// RH - Result of internal temperatur 2 measurement SD-ADC result of internal temperature 2 measurement.
    #define RESULT__DEFAULT					 0b0000000000000000	// Default 
...
extern const subreg_item  CONFIG__NODE_ID; 							// RWO - Address (ID) of the node, distributed during enumeration NODE_ID = 0 --> iso UART signals are not forwardedNODE_ID = 63 --> reserved for broadcast commands
extern const subreg_item  CONFIG__EN_ALL_ADC; 						// RW - Enable all ADCs If this bit is set, PCVM is done for each channel, independent ofPART_CONFIG setup.
...
extern const subreg_item  CONFIG__FN; 								// RWO - Final Node The final node in stack must have this bit set. If final node does nothave FN set, no reply frame on broadcast will be sent.
...
extern const subreg_item  GPIO__PWM_PWM1; 							// RW - PWM 1 enable PWM function
    ...
    #define PWM_PWM1__ENABLE				 0b0000000010000000	// If DIR_PWM1 = 1, then PWM output regarding PWM_GPIO register and OUT_PWM1 is ignored. If DIR_PWM1 = 0, GPIO inputand no PWM function.
...
extern const subreg_item  GPIO__PWM_PWM0; 							// RW - PWM 0 enable PWM function
    ...
    #define PWM_PWM0__ENABLE				 0b0000100000000000	// If DIR_PWM0= 1, then PWM output regarding PWM_GPIO register and OUT_PWM0 is ignored. If DIR_PWM0= 0, GPIO inputand no PWM function.
...
extern const subreg_item  WDOG_CNT__MAIN_CNT; 						// RH - Main counter Used to enable host controller to measure the main oscillatorfrequency. LSB = t Count_LSB. Reset by write access to WD_CNT ifRR_SYNC=1.
...
extern const subreg_item  BAL_CNT_0__BAL_CNT_0; 					// RW - Balancing counter to switch off balancing after a certain time, counter is (re-)started by a write access to WDOG_CNT when EXT_WD=1.
    #define BAL_CNT_0__VAL_1				 0b0000000000000001	// 7.5 min 
    #define BAL_CNT_0__VAL_31				 0b0000000000011111	// 3.87 h 
extern const subreg_item  BAL_CNT_0__BAL_CNT_1; 					// RW - Balancing counter to switch off balancing after a certain time, counter is (re-)started by a write access to WDOG_CNT when EXT_WD=1.
    #define BAL_CNT_1__VAL_1				 0b0000000000100000	// 7.5 min 
    #define BAL_CNT_1__VAL_31				 0b0000001111100000	// 3.87 h 
extern const subreg_item  BAL_CNT_0__BAL_CNT_2; 					// RW - Balancing counter to switch off balancing after a certain time, counter is (re-)started by a write access to WDOG_CNT when EXT_WD=1.
...
extern const subreg_item  BAL_CNT_1__BAL_CNT_3; 					// RW - Balancing counter to switch off balancing after a certain time, counter is (re-)started by a write access to WDOG_CNT when EXT_WD=1.
    #define BAL_CNT_3__VAL_1				 0b0000000000000001	// 7.5 min 
    #define BAL_CNT_3__VAL_31				 0b0000000000011111	// 3.87 h 
extern const subreg_item  BAL_CNT_1__BAL_CNT_4; 					// RW - Balancing counter to switch off balancing after a certain time, counter is (re-)started by a write access to WDOG_CNT when EXT_WD=1.
    #define BAL_CNT_4__VAL_1				 0b0000000000100000	// 7.5 min 
    #define BAL_CNT_4__VAL_31				 0b0000001111100000	// 3.87 h 
extern const subreg_item  BAL_CNT_1__BAL_CNT_5; 					// RW - Balancing counter to switch off balancing after a certain time, counter is (re-)started by a write access to WDOG_CNT when EXT_WD=1.
...
extern const subreg_item  BAL_CNT_2__BAL_CNT_6; 					// RW - Balancing counter to switch off balancing after a certain time, counter is (re-)started by a write access to WDOG_CNT when EXT_WD=1.
    #define BAL_CNT_6__VAL_1				 0b0000000000000001	// 7.5 min 
    #define BAL_CNT_6__VAL_31				 0b0000000000011111	// 3.87 h 
extern const subreg_item  BAL_CNT_2__BAL_CNT_7; 					// RW - Balancing counter to switch off balancing after a certain time, counter is (re-)started by a write access to WDOG_CNT when EXT_WD=1.
    #define BAL_CNT_7__VAL_1				 0b0000000000100000	// 7.5 min 
    #define BAL_CNT_7__VAL_31				 0b0000001111100000	// 3.87 h 
extern const subreg_item  BAL_CNT_2__BAL_CNT_8; 					// RW - Balancing counter to switch off balancing after a certain time, counter is (re-)started by a write access to WDOG_CNT when EXT_WD=1.
...
extern const subreg_item  BAL_CNT_3__BAL_CNT_9; 					// RW - Balancing counter to switch off balancing after a certain time, counter is (re-)started by a write access to WDOG_CNT when EXT_WD=1.
    #define BAL_CNT_9__VAL_1				 0b0000000000000001	// 7.5 min 
    #define BAL_CNT_9__VAL_31				 0b0000000000011111	// 3.87 h 
extern const subreg_item  BAL_CNT_3__BAL_CNT_10; 					// RW - Balancing counter to switch off balancing after a certain time, counter is (re-)started by a write access to WDOG_CNT when EXT_WD=1.
    #define BAL_CNT_10__VAL_1				 0b0000000000100000	// 7.5 min 
    #define BAL_CNT_10__VAL_31				 0b0000001111100000	// 3.87 h 
extern const subreg_item  BAL_CNT_3__BAL_CNT_11; 					// RW - Balancing counter to switch off balancing after a certain time, counter is (re-)started by a write access to WDOG_CNT when EXT_WD=1.
...
#define GEN_DIAG_READ_ONLY_FLAGS (uint16_t)(UART_WAKEUP__UART | MOT_MOB_N__TOP | BAL_ACTIVE__ON | LOCK_MEAS__MEAS_ON | RR_ACTIVE__ON)
...
// Function prototypes
//****************************************************************************
void setRegisterCacheDataByIndex_tle9012(uint8_t nodeID, uint8_t reg_index, uint16_t reg_data);
