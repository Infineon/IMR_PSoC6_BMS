...
//****************************************************************************
#define DEBUGPRINTF_QUEUE(p_frm, ...)			//DEBUGPRINTF(p_frm, ##__VA_ARGS__) // De/activate common debug messages of queuing system
#define DEBUGPRINTF_QUEUE_PERF(p_frm, ...)		//DEBUGPRINTF(p_frm, ##__VA_ARGS__)	// De/activate performance debug messages of queuing system
uint8_t queue_index_latest = 0;  // The index of the last item in the queue, new items will be added after this index
uint8_t queue_index_current = 0; // The index of the currently processed item in the queue
queue_item queue_items[QUEUE_ITEMS_MAX] = {{0,0,0,0,0,WAIT_FOR_NEXT_ITEM}}; // First item in queue must be set to wait for next item
...
//****************************************************************************
//****************************************************************************
// resetQueue - Resets the indices, the first queue item and the last CommandSent
//****************************************************************************
void resetQueue(){
...
QueueResult addToQueue_tle9012(uint8_t nodeID, uint8_t isWrite, uint8_t registerIdx, uint16_t data, uint8_t isBroadcast){
	...
	if(queue_index_latest != queue_index_current){
		...
		//DEBUGPRINTF_QUEUE("\tAdd queue item: id %d, isWrite %d, register %d, data %d\r\n", queue_items[queue_index_latest].nodeID, queue_items[queue_index_latest].isWrite, queue_items[queue_index_latest].regIndex, queue_items[queue_index_latest].data);
		DEBUGPRINTF_QUEUE("\tAdd  queue item at index %d: Register %d in state %d\r\n", queue_index_latest, queue_items[queue_index_latest].regIndex, queue_items[queue_index_latest].state);
...
//****************************************************************************
// Manages queue items - Resets the ready bit of the register, sends the request
// and waits till ready bit is set again (from FIFO interpreter). After this it goes on with the next
...
QueueResult tle9012_manageQueue(uint8_t keepAlive){
	...
	DEBUGPRINTF_QUEUE("tle9012_manageQueue: current index %d, latest index %d, state %d\r\n", queue_index_current, queue_index_latest, queue_items[queue_index_current].state);
	...
	if(TIMER_COUNTER_1MS > (timeLastWatchdogRefresh + TLE9012_WATCHDOG_MAXIMUM_TIME)){
		DEBUGPRINTF("[ERROR]: Last TLE9012 watchdog refresh was %ld ms ago. Will reinitialize to wake devices!\r\n", TIMER_COUNTER_1MS - timeLastWatchdogRefresh);

		// Reinitialize all tle9012 ICs
		tle9012_deviceInit(1);

		// Return that device was reinitialized so that main application can reconfig the TL9012 as seen fit
	...
	if(queue_items[queue_index_current].state == WAIT_FOR_RX){
		DEBUGPRINTF_QUEUE("\tWAIT_FOR_RX on queue index %d: NodeId %d, RegisterIdx %d, Write %d, ready %d\r\n", queue_index_current, queue_items[queue_index_current].nodeID, queue_items[queue_index_current].regIndex, queue_items[queue_index_current].isWrite, checkRegisterReady_tle9012(queue_items[queue_index_current].nodeID, queue_items[queue_index_current].regIndex, queue_items[queue_index_current].isBroadcast));
		// Check ready bit of register until it is ready
		if(checkRegisterReady_tle9012(queue_items[queue_index_current].nodeID, queue_items[queue_index_current].regIndex, queue_items[queue_index_current].isBroadcast)){
			//DEBUGPRINTF_QUEUE("\tReceived queue index %d: NodeId %d, RegisterIdx %d, Write %d, Data 0x%04X\r\n", queue_index_current, queue_items[queue_index_current].nodeID, queue_items[queue_index_current].regIndex, queue_items[queue_index_current].isWrite, getRegisterCacheDataByIndex_tle9012(&tle9012_nodes[queue_items[queue_index_current].nodeID-1], queue_items[queue_index_current].regIndex));

			// Set register index as not queued
			resetRegisterQueued_tle9012(queue_items[queue_index_current].nodeID, queue_items[queue_index_current].regIndex, queue_items[queue_index_current].isBroadcast);

			// If there is another item in queue execute it ... TODO: Code restructure (this code is also in next block...)
			if(queue_index_latest != queue_index_current){
		...
		//DEBUGPRINTF_QUEUE("\r\nTimeout %ld %ld \r\n", TIMER_COUNTER_1MS, tle9012_nodes[queue_items[queue_index_current].nodeID-1].timeLastCommandSent);
		else if(TIMER_COUNTER_1MS > (timeLastQueueCommandSent + TIMEOUT_COMMAND)){
			DEBUGPRINTF_QUEUE("\r\n[ERROR]: Timeout tle9012_manageQueue took %ld ms (max=%d)\r\n", (uint32_t)(TIMER_COUNTER_1MS - timeLastQueueCommandSent), TIMEOUT_COMMAND);
			// Mark register as ready
			setRegisterReady_tle9012(queue_items[queue_index_current].nodeID, queue_items[queue_index_current].regIndex, queue_items[queue_index_current].isBroadcast);
			// Mark register as error
			setRegisterError_tle9012(queue_items[queue_index_current].nodeID, queue_items[queue_index_current].regIndex, queue_items[queue_index_current].isBroadcast);
			// Set register index as not queued
			resetRegisterQueued_tle9012(queue_items[queue_index_current].nodeID, queue_items[queue_index_current].regIndex, queue_items[queue_index_current].isBroadcast);
	...
	if(queue_items[queue_index_current].state == EXECUTE_TX){
		...
		if(keepAlive == 1 && TIMER_COUNTER_1MS > (timeLastWatchdogRefresh + TLE9012_WATCHDOG_REFRESH_TIME)){
			sendWatchdogUpdate(1, 0); // TODO: check if this works as broadcast
		}
		else{
			// Send command
			DEBUGPRINTF_QUEUE("\tSend queue index %d: NodeId %d, RegisterIdx %d, Write %d, Data 0x%04X\r\n", queue_index_current, queue_items[queue_index_current].nodeID, queue_items[queue_index_current].regIndex, queue_items[queue_index_current].isWrite, queue_items[queue_index_current].data);
			if(queue_items[queue_index_current].isWrite == 1){
				sendWrite(tle9012_nodes, reg_items[queue_items[queue_index_current].regIndex].address, queue_items[queue_index_current].data, queue_items[queue_index_current].isBroadcast);
			}
			else{
				sendRead(tle9012_nodes, reg_items[queue_items[queue_index_current].regIndex].address, queue_items[queue_index_current].isBroadcast);
			}

			// Mark register as not ready
			resetRegisterReady_tle9012(queue_items[queue_index_current].nodeID, queue_items[queue_index_current].regIndex, queue_items[queue_index_current].isBroadcast);
			// Mark register as no error
			//resetRegisterError_tle9012(queue_items[queue_index_current].nodeID, queue_items[queue_index_current].regIndex, queue_items[queue_index_current].isBroadcast);
