...
// TARGET SPECIFICATION FOR CY8C6245
	...
	// Only needed for high level access
	// (Do not use here - will break low level access)
	...
	const cy_stc_scb_uart_config_t uartConfig = {
		...
		.txFifoTriggerLevel         = 0UL,//(CY_SCB_FIFO_SIZE/2 - 1),
	...
	void UART_Isr(void)
	{
		...
		// When used will break many other things because ISR takes to long
		// only use for debug when really needed
		/*DEBUGPRINTF("\r\n\tUART ISR: Bytes in FIFO %ld, FIFO status %ld, "
				"Receive status %ld\r\n",
				Cy_SCB_UART_GetNumInRxFifo(SCB3),
				Cy_SCB_UART_GetRxFifoStatus(SCB3),
				Cy_SCB_UART_GetReceiveStatus(SCB3, &uartContext));*/
	...
	cy_rslt_t uartInit()
	{
		...
		// Debug - following parameters are constructed from many parts.
		// To see the actual error code they are displayed at runtime when needed
		...
		cy_en_scb_uart_status_t res_init =
				Cy_SCB_UART_Init(SCB3, &uartConfig, NULL); //&uartContext
		if (res_init != CY_SCB_UART_SUCCESS) {
			DEBUGPRINTF("\tFailed UART SCB TLE9012: %d\r\n",
		...
		cy_en_sysclk_status_t res_clk =
				Cy_SysClk_PeriphAssignDivider(PCLK_SCB3_CLOCK,
						UART_CLK_DIV_TYPE, UART_CLK_DIV_NUMBER);
		if (res_clk != CY_SYSCLK_SUCCESS) {
			DEBUGPRINTF("\tFailed UART SCB TLE9012 ASSIGN CLOCK: %d\r\n",
					(int)(uint8_t)res_clk);
		}

		/* UART desired baud rate is 115200 bps (Standard mode).
		* The UART baud rate = (clk_scb / Oversample).
		* For clk_peri = 50 MHz, select divider value 36 and
		* get SCB clock = (50 MHz / 36) = 1,389 MHz.
		...
		*/
		...
		res_clk = Cy_SysClk_PeriphSetDivider(UART_CLK_DIV_TYPE,
				UART_CLK_DIV_NUMBER, 35UL);*/
		res_clk = Cy_SysClk_PeriphSetFracDivider(UART_CLK_DIV_TYPE,
				UART_CLK_DIV_NUMBER, 5U, 8U);
		if (res_clk != CY_SYSCLK_SUCCESS) {
			DEBUGPRINTF("\tFailed UART SCB TLE9012 ASSIGN CLOCK "
					"SetFracDivider: %d\r\n", (int)(uint8_t)res_clk);
		}
		res_clk = Cy_SysClk_PeriphEnableDivider(UART_CLK_DIV_TYPE,
				UART_CLK_DIV_NUMBER);
		if (res_clk != CY_SYSCLK_SUCCESS) {
			DEBUGPRINTF("\tFailed UART SCB TLE9012 CLOCK EnableDivider: %d\r\n",
					(int)(uint8_t)res_clk);
		}
