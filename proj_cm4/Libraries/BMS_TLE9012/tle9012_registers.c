...
// Items for every subregister/setting containing: main register index,
// bitmask of specific setting and label
...
const subreg_item  OL_OV_THR__OV_THR = 					{1 ,0b0000001111111111, "OV_THR"};				 // RW - Overvoltage fault threshold 10-bit overvoltage fault threshold.
																										 // Battery input voltages (U0 to U11)are tested for overvoltage with given value.
																										 // OV error is detected andindicated in GEN_DIAG register if cell voltage is higher than OV faultthreshold.
const subreg_item  OL_OV_THR__OL_THR_MAX = 				{1 ,0b1111110000000000, "OL_THR_MAX"};			 // RW - Open load maximum voltage drop threshold (LSB8) 6-bit (LSB8) to define the maximum threshold for the voltage dropwhile OL-diagnostics (I OL_DIAG * R F).
																										 // If voltage drop > OL_THR_MAX, theOLx bit of channel x is set.
const subreg_item  OL_UV_THR__UV_THR = 					{2 ,0b0000001111111111, "UV_THR"};				 // RW - Undervoltage fault threshold 10-bit undervoltage fault threshold.
																										 // Battery input voltages (U0 to U11)are tested for undervoltage with given value.
																										 // UV error is detected andindicated in GEN_DIAG register if cell voltage is lower than UV faultthreshold.
const subreg_item  OL_UV_THR__OL_THR_MIN = 				{2 ,0b1111110000000000, "OL_THR_MIN"};			 // RW - Open load minimum voltage drop threshold (LSB8) 6-bit (LSB8) to define the minimum threshold for the voltage drop whileOL-diagnostics (I OL_DIAG * R F).
																										 // If voltage drop < OL_THR_MIN, the OLxbit of channel x is set.
const subreg_item  TEMP_CONF__EXT_OT_THR = 				{3 ,0b0000001111111111, "EXT_OT_THR"};			 // RW - External overtemperature threshold 10-bit overtemperature fault threshold.
																										 // Overtemperature error isdetected and indicated in GEN_DIAG register if the externaltemperature result is lower than the fault threshold. Balancing isdeactivated.
...
const subreg_item  INT_OT_WARN_CONF__INT_OT_THR = 		{4 ,0b0000001111111111, "INT_OT_THR"};			 // RW - Internal overtemperature threshold 10-bit overtemperature fault threshold.
																										 // Overtemperature error isdetected and indicated in GEN_DIAG register if the internal temperatureresult is LOWER than the fault threshold. Balancing is deactivated.
const subreg_item  RR_ERR_CNT__NR_ERR = 				{5 ,0b0000000000000111, "NR_ERR"};				 // RW - Number of errors Number of consecutive detected errors before error is valid and setin GEN_DIAG and individual fault registers.
																										 // Only used for faults wherecounter NR_ERR is active (this can be set in register NR_ERR_MASK).
																										 // Please note: The register CRC errors as well as the internal IC errors donot have an error counter.
...
const subreg_item  GEN_DIAG__LOCK_MEAS = 				{8 ,0b0000000000001000, "LOCK_MEAS"};			 // RH - Lock measurement
																										 // This bit indicates an ongoing PCVM, SCVM, BVM or AVM measurement or a delayed RR.
...
const subreg_item  GEN_DIAG__OL_ERR = 					{8 ,0b0000000010000000, "OL_ERR"};				 // ROCWL - Open load error
																										 // Please note: Resetting the error here resets also the respective detailederror register.
const subreg_item  GEN_DIAG__INT_IC_ERR = 				{8 ,0b0000000100000000, "INT_IC_ERR"};			 // ROCW - Internal IC error
const subreg_item  GEN_DIAG__REG_CRC_ERR = 				{8 ,0b0000001000000000, "REG_CRC_ERR"};			 // ROCWL - Register CRC error
																										 // Please note: Resetting the error here resets also the respective detailederror register.
const subreg_item  GEN_DIAG__EXT_T_ERR = 				{8 ,0b0000010000000000, "EXT_T_ERR"};			 // ROCWL - External temperature error
																										 // Please note: Resetting the error here resets also the respective detailederror register.
const subreg_item  GEN_DIAG__INT_OT = 					{8 ,0b0000100000000000, "INT_OT"};				 // ROCW - Internal temperature (OT) error
const subreg_item  GEN_DIAG__CELL_UV = 					{8 ,0b0001000000000000, "CELL_UV"};				 // ROCWL - Cell undervoltage (UV) error
																										 // Please note: Resetting the error here resets also the respective detailederror register.
const subreg_item  GEN_DIAG__CELL_OV = 					{8 ,0b0010000000000000, "CELL_OV"};				 // ROCWL - Cell overvoltage (OV) error
																										 // Please note: Resetting the error here resets also the respective detailederror register.
const subreg_item  GEN_DIAG__BAL_ERR_UC = 				{8 ,0b0100000000000000, "BAL_ERR_UC"};			 // ROCWL - Balancing error undercurrent (will be reset in sleep mode)
																										 // Please note: Resetting the error here resets also the respective detailederror register.
const subreg_item  GEN_DIAG__BAL_ERR_OC = 				{8 ,0b1000000000000000, "BAL_ERR_OC"};			 // ROCWL - Balancing error overcurrent (will be reset in sleep mode)
																										 // Please Note: Resetting the error here resets also the respective detailederror register.
...
const subreg_item  REG_CRC_ERR__ADDR = 					{13 ,0b0000000001111111, "ADDR"};				 // ROCW - Register address of register where CRC check failed Register address of CRC check fail,
...
const subreg_item  BAL_CURR_THR__OC_THR = 				{17 ,0b0000000011111111, "OC_THR"};				 // RW - Overcurrent fault threshold 8-bit to define the maximum voltage drop during balancingdiagnostics.
																										 // If the voltage drop (I Bal*R F) > OC_THR the overcurrentis detected.
const subreg_item  BAL_CURR_THR__UC_THR = 				{17 ,0b1111111100000000, "UC_THR"};				 // RW - Undercurrent fault threshold 8-bit to define the minimum voltage drop during balancing diagnostics.
...
const subreg_item* subreg_items[NUM_SUBREGISTERS] = {
	&PART_CONFIG__EN_CELLi, &PART_CONFIG__EN_CELL11,
	&OL_OV_THR__OV_THR, &OL_OV_THR__OL_THR_MAX, &OL_UV_THR__UV_THR,
	&OL_UV_THR__OL_THR_MIN, &TEMP_CONF__EXT_OT_THR, &TEMP_CONF__I_NTC,
	&TEMP_CONF__NR_TEMP_SENSE, &INT_OT_WARN_CONF__INT_OT_THR,
	&RR_ERR_CNT__NR_ERR, &RR_ERR_CNT__NR_EXT_TEMP_START,
	&RR_ERR_CNT__RR_SLEEP_CNT, &RR_CONFIG__RR_CNT, &RR_CONFIG__RR_SYNC,
	&RR_CONFIG__M_NR_ERR_ADC_ERR, &RR_CONFIG__M_NR_ERR_OL_ERR,
	&RR_CONFIG__M_NR_ERR_EXT_T_ERR, &RR_CONFIG__M_NR_ERR_INT_OT,
	&RR_CONFIG__M_NR_ERR_CELL_UV, &RR_CONFIG__M_NR_ERR_CELL_OV,
	&RR_CONFIG__M_NR_ERR_BAL_UC, &RR_CONFIG__M_NR_ERR_BAL_OC,
	&FAULT_MASK__ERR_PIN, &FAULT_MASK__M_ADC_ERR, &FAULT_MASK__M_OL_ERR,
	&FAULT_MASK__M_INT_IC_ERR, &FAULT_MASK__M_REG_CRC_ERR,
	&FAULT_MASK__M_EXT_T_ERR, &FAULT_MASK__M_INT_OT,
	&FAULT_MASK__M_CELL_UV, &FAULT_MASK__M_CELL_OV,
	&FAULT_MASK__M_BAL_ERR_UC, &FAULT_MASK__M_BAL_ERR_OC,
	&GEN_DIAG__UART_WAKEUP, &GEN_DIAG__MOT_MOB_N, &GEN_DIAG__BAL_ACTIVE,
	&GEN_DIAG__LOCK_MEAS, &GEN_DIAG__RR_ACTIVE, &GEN_DIAG__PS_ERR_SLEEP,
	&GEN_DIAG__ADC_ERR, &GEN_DIAG__OL_ERR, &GEN_DIAG__INT_IC_ERR,
	&GEN_DIAG__REG_CRC_ERR, &GEN_DIAG__EXT_T_ERR, &GEN_DIAG__INT_OT,
	&GEN_DIAG__CELL_UV, &GEN_DIAG__CELL_OV, &GEN_DIAG__BAL_ERR_UC,
	&GEN_DIAG__BAL_ERR_OC, &CELL_UV__UV_i, &CELL_OV__OV_i,
	&EXT_TEMP_DIAG__OPEN0, &EXT_TEMP_DIAG__SHORT0, &EXT_TEMP_DIAG__OT0,
	&EXT_TEMP_DIAG__OPEN1, &EXT_TEMP_DIAG__SHORT1, &EXT_TEMP_DIAG__OT1,
	&EXT_TEMP_DIAG__OPEN2, &EXT_TEMP_DIAG__SHORT2, &EXT_TEMP_DIAG__OT2,
	&EXT_TEMP_DIAG__OPEN3, &EXT_TEMP_DIAG__SHORT3, &EXT_TEMP_DIAG__OT3,
	&EXT_TEMP_DIAG__OPEN4, &EXT_TEMP_DIAG__SHORT4, &EXT_TEMP_DIAG__OT4,
	&DIAG_OL__OL_i, &REG_CRC_ERR__ADDR, &CELL_UV_DAC_COMP__UV_i,
	&CELL_OV_DAC_COMP__OV_i, &OP_MODE__PD, &OP_MODE__EXT_WD,
	&OP_MODE__SLEEP_REG_RESET, &OP_MODE__I_DIAG_EN, &OP_MODE__LR_TIME,
	&BAL_CURR_THR__OC_THR, &BAL_CURR_THR__UC_THR,
	&AVM_CONFIG__TEMP_MUX_DIAG_SEL, &AVM_CONFIG__AVM_TMP0_MASK,
	&AVM_CONFIG__AVM_TMP1_MASK, &AVM_CONFIG__AVM_TMP2_MASK,
	&AVM_CONFIG__AVM_TMP3_MASK, &AVM_CONFIG__AVM_TMP4_MASK,
	&AVM_CONFIG__AUX_BIPOLAR, &AVM_CONFIG__R_DIAG,
	&AVM_CONFIG__R_DIAG_SEL_0, &AVM_CONFIG__R_DIAG_SEL_1,
	&AVM_CONFIG__R_DIAG_CUR_SRC, &MEAS_CTRL__CVM_DEL, &MEAS_CTRL__PBOFF,
	&MEAS_CTRL__SCVM_START, &MEAS_CTRL__AVM_START, &MEAS_CTRL__BVM_MODE,
	&MEAS_CTRL__BVM_START, &MEAS_CTRL__CVM_MODE, &MEAS_CTRL__PCVM_START,
	&PCVM_0__RESULT, &PCVM_1__RESULT, &PCVM_2__RESULT, &PCVM_3__RESULT,
	&PCVM_4__RESULT, &PCVM_5__RESULT, &PCVM_6__RESULT, &PCVM_7__RESULT,
	&PCVM_8__RESULT, &PCVM_9__RESULT, &PCVM_10__RESULT, &PCVM_11__RESULT,
	&SCVM_HIGH__UPD_CNT, &SCVM_HIGH__RESULT, &SCVM_LOW__UPD_CNT,
	&SCVM_LOW__RESULT, &STRESS_PCVM__STRESS_CORRECTION, &BVM__RESULT,
	&EXT_TEMP_0__RESULT, &EXT_TEMP_0__INTC, &EXT_TEMP_0__PULLDOWN,
	&EXT_TEMP_0__VALID, &EXT_TEMP_0__PD_ERR, &EXT_TEMP_1__RESULT,
	&EXT_TEMP_1__INTC, &EXT_TEMP_1__PULLDOWN, &EXT_TEMP_1__VALID,
	&EXT_TEMP_1__PD_ERR, &EXT_TEMP_2__RESULT, &EXT_TEMP_2__INTC,
	&EXT_TEMP_2__PULLDOWN, &EXT_TEMP_2__VALID, &EXT_TEMP_2__PD_ERR,
	&EXT_TEMP_3__RESULT, &EXT_TEMP_3__INTC, &EXT_TEMP_3__PULLDOWN,
	&EXT_TEMP_3__VALID, &EXT_TEMP_3__PD_ERR, &EXT_TEMP_4__RESULT,
	&EXT_TEMP_4__INTC, &EXT_TEMP_4__PULLDOWN, &EXT_TEMP_4__VALID,
	&EXT_TEMP_4__PD_ERR, &EXT_TEMP_R_DIAG__RESULT, &EXT_TEMP_R_DIAG__INTC,
	&EXT_TEMP_R_DIAG__CUR_SRC, &EXT_TEMP_R_DIAG__VALID, &INT_TEMP__RESULT,
	&INT_TEMP__VALID, &MULTI_READ__RES, &MULTI_READ_CFG__PCVM_SEL,
	&MULTI_READ_CFG__BVM_SEL, &MULTI_READ_CFG__EXT_TEMP_SEL,
	&MULTI_READ_CFG__EXT_TEMP_R_SEL, &MULTI_READ_CFG__INT_TEMP_SEL,
	&MULTI_READ_CFG__SCVM_SEL, &MULTI_READ_CFG__STRESS_PCVM_SEL,
	&BAL_DIAG_OC__OC_i, &BAL_DIAG_UC__UC_i, &INT_TEMP_2__RESULT,
	&INT_TEMP_2__VALID, &CONFIG__NODE_ID, &CONFIG__EN_ALL_ADC,
	&CONFIG__FN, &GPIO__IN_GPIO0, &GPIO__OUT_GPIO0, &GPIO__DIR_GPIO0,
	&GPIO__IN_GPIO1, &GPIO__OUT_GPIO1, &GPIO__DIR_GPIO1,
	&GPIO__IN_PWM1, &GPIO__PWM_PWM1, &GPIO__OUT_PWM1, &GPIO__DIR_PWM1,
	&GPIO__IN_PWM0, &GPIO__PWM_PWM0, &GPIO__OUT_PWM0, &GPIO__DIR_PWM0,
	&GPIO__VIO_UV, &GPIO_PWM__PWM_DUTY_CYCLE, &GPIO_PWM__PWM_PERIOD,
	&ICVID__VERSION_ID, &ICVID__MANUFACTURER_ID, &MAILBOX__DATA,
	&CUSTOMER_ID_0__DATA, &CUSTOMER_ID_1__DATA, &WDOG_CNT__WD_CNT,
	&WDOG_CNT__MAIN_CNT, &SCVM_CONFIG__EN_SCVMi, &STRESS_AUX__STRESS_CORRECTION,
	&BAL_PWM__BAL_PWM, &BAL_CNT_0__BAL_CNT_0, &BAL_CNT_0__BAL_CNT_1,
	&BAL_CNT_0__BAL_CNT_2, &BAL_CNT_1__BAL_CNT_3, &BAL_CNT_1__BAL_CNT_4,
	&BAL_CNT_1__BAL_CNT_5, &BAL_CNT_2__BAL_CNT_6, &BAL_CNT_2__BAL_CNT_7,
	&BAL_CNT_2__BAL_CNT_8, &BAL_CNT_3__BAL_CNT_9, &BAL_CNT_3__BAL_CNT_10,
	&BAL_CNT_3__BAL_CNT_11,
};
...
uint8_t checkRegisterReady_tle9012(uint8_t nodeID, uint8_t reg_index,
	...
	if(reg_index == MULTI_READ){
		...
		for(uint8_t i = 0; i < multi_read_cfg_reg_idx_list_num; i++){
			result &= checkRegisterReady_tle9012(nodeID,
					multi_read_cfg_reg_idx_list[i], isBroadcast);
		}
		//DEBUGPRINTF_BITMAP_CHANGE("MULTI check result %d\r\n", result);
		return result;
	}
	else if(isBroadcast == 0)
		return (tle9012_nodes[nodeID-1].reg_items_ready_bitmap &
				((uint64_t)1 << ((uint64_t)reg_index))) != 0;
	else{
		uint64_t result = UINT64_MAX;
		uint8_t curNodeIdx = 0;
		do{
			result &= (tle9012_nodes[curNodeIdx].reg_items_ready_bitmap &
					((uint64_t)1 << ((uint64_t)reg_index)));
			DEBUGPRINTF_BITMAP_CHANGE("\t\t\tCheck ready register index "
					"%2d of Node index %2d: %d\r\n",
...
void setRegisterReady_tle9012(uint8_t nodeID, uint8_t reg_index,
	...
	if(reg_index == MULTI_READ){
		for(uint8_t i = 0; i < multi_read_cfg_reg_idx_list_num; i++){
			setRegisterReady_tle9012(nodeID,
					multi_read_cfg_reg_idx_list[i], isBroadcast);
		}
	}
	else if(isBroadcast == 0){
		DEBUGPRINTF_BITMAP_CHANGE("\t\t\tRegister index "
				"%2d of NodeID %2d: Ready\r\n", reg_index, nodeID);
		tle9012_nodes[nodeID-1].reg_items_ready_bitmap |=
				((uint64_t)1 << ((uint64_t)reg_index));
	}
	else{
		uint8_t curNodeIdx = 0;
		do{
			DEBUGPRINTF_BITMAP_CHANGE("\t\t\tRegister index "
					"%2d of Node index %2d: Ready\r\n",
					reg_index, curNodeIdx);
			tle9012_nodes[curNodeIdx].reg_items_ready_bitmap |=
					((uint64_t)1 << ((uint64_t)reg_index));
			curNodeIdx++;
		}while(curNodeIdx < TLE9012_NODES_SIZE);
	}
...
void resetRegisterReady_tle9012(uint8_t nodeID, uint8_t reg_index,
	...
	if(reg_index == MULTI_READ){
		for(uint8_t i = 0; i < multi_read_cfg_reg_idx_list_num; i++){
			resetRegisterReady_tle9012(nodeID,
					multi_read_cfg_reg_idx_list[i], isBroadcast);
		}
	}
	else if(isBroadcast == 0){
		DEBUGPRINTF_BITMAP_CHANGE("\t\t\tRegister index "
				"%2d of NodeID %2d: NOT Ready\r\n", reg_index, nodeID);
		tle9012_nodes[nodeID-1].reg_items_ready_bitmap &=
				~((uint64_t)1 << ((uint64_t)reg_index));
	}
	else{
...
uint8_t checkRegisterError_tle9012(uint8_t nodeID, uint8_t reg_index,
	...
	if(reg_index == MULTI_READ){
		uint8_t result = 0;
		for(uint8_t i = 0; i < multi_read_cfg_reg_idx_list_num; i++){
			result |= checkRegisterError_tle9012(nodeID,
					multi_read_cfg_reg_idx_list[i], isBroadcast);
		}
		return result;
	}
	else if(isBroadcast == 0)
		return (tle9012_nodes[nodeID-1].reg_items_error_bitmap &
				((uint64_t)1 << ((uint64_t)reg_index))) != 0;
	else{
		uint64_t result = 0;
		uint8_t curNodeIdx = 0;
		do{
			result |= (tle9012_nodes[curNodeIdx].reg_items_error_bitmap &
					((uint64_t)1 << ((uint64_t)reg_index)));
			DEBUGPRINTF_BITMAP_CHANGE("\t\t\tCheck error register index "
					"%2d of Node index %2d: %d\r\n",
					reg_index, curNodeIdx, result != 0);
			curNodeIdx++;
		}while(curNodeIdx < TLE9012_NODES_SIZE);
...
void setRegisterError_tle9012(uint8_t nodeID, uint8_t reg_index,
	...
	if(reg_index == MULTI_READ){
		for(uint8_t i = 0; i < multi_read_cfg_reg_idx_list_num; i++){
			setRegisterError_tle9012(nodeID,
					multi_read_cfg_reg_idx_list[i], isBroadcast);
		}
	}
	else if(isBroadcast == 0){
		DEBUGPRINTF_BITMAP_CHANGE("\t\t\tRegister index "
				"%2d of Node ID %2d: Error\r\n", reg_index, nodeID);
		tle9012_nodes[nodeID-1].reg_items_error_bitmap |=
				((uint64_t)1 << ((uint64_t)reg_index));
	}
	else{
		uint8_t curNodeIdx = 0;
		do{
			DEBUGPRINTF_BITMAP_CHANGE("\t\t\tRegister index "
					"%2d of Node index %2d: Error\r\n", reg_index, curNodeIdx);
			tle9012_nodes[curNodeIdx].reg_items_error_bitmap |=
					((uint64_t)1 << ((uint64_t)reg_index));
			curNodeIdx++;
		}while(curNodeIdx < TLE9012_NODES_SIZE);
	}
...
void resetRegisterError_tle9012(uint8_t nodeID, uint8_t reg_index,
	...
	if(reg_index == MULTI_READ){
		for(uint8_t i = 0; i < multi_read_cfg_reg_idx_list_num; i++){
			resetRegisterError_tle9012(nodeID,
					multi_read_cfg_reg_idx_list[i], isBroadcast);
		}
	}
	else if(isBroadcast == 0){
		DEBUGPRINTF_BITMAP_CHANGE("\t\t\tRegister index "
				"%2d of Node ID %2d: NO Error\r\n", reg_index, nodeID);
		tle9012_nodes[nodeID-1].reg_items_error_bitmap &=
				~((uint64_t)1 << ((uint64_t)reg_index));
	}
	else{
...
uint8_t checkRegisterQueued_tle9012(uint8_t nodeID, uint8_t reg_index,
	...
	if(reg_index == MULTI_READ){
		uint8_t result = 1;
		for(uint8_t i = 0; i < multi_read_cfg_reg_idx_list_num; i++){
			result &= checkRegisterQueued_tle9012(nodeID,
					multi_read_cfg_reg_idx_list[i], isBroadcast);
		}
		return result;
	}
	else if(isBroadcast == 0)
		return (tle9012_nodes[nodeID-1].reg_items_queued_bitmap &
				((uint64_t)1 << ((uint64_t)reg_index))) != 0;
	else{
		uint64_t result = UINT64_MAX;
		uint8_t curNodeIdx = 0;
		do{
			result &= (tle9012_nodes[curNodeIdx].reg_items_queued_bitmap &
					((uint64_t)1 << ((uint64_t)reg_index)));
			/*DEBUGPRINTF_BITMAP_CHANGE("\t\t\tCheck Queued register index "
					"%2d of Node index %2d: %d\r\n",
					reg_index, curNodeIdx, result != 0);*/
			curNodeIdx++;
		}while(curNodeIdx < TLE9012_NODES_SIZE);
...
void setRegisterQueued_tle9012(uint8_t nodeID, uint8_t reg_index,
	...
	if(reg_index == MULTI_READ){
		for(uint8_t i = 0; i < multi_read_cfg_reg_idx_list_num; i++){
			setRegisterQueued_tle9012(nodeID,
					multi_read_cfg_reg_idx_list[i], isBroadcast);
		}
	}
	else if(isBroadcast == 0){
		DEBUGPRINTF_BITMAP_CHANGE("\t\t\tRegister index "
				"%2d of Node ID %2d: Queued\r\n", reg_index, nodeID);
		tle9012_nodes[nodeID-1].reg_items_queued_bitmap |=
				((uint64_t)1 << ((uint64_t)reg_index));
	}
	else{
		uint8_t curNodeIdx = 0;
		do{
			DEBUGPRINTF_BITMAP_CHANGE("\t\t\tRegister index "
					"%2d of Node index %2d: Queued\r\n", reg_index, curNodeIdx);
			tle9012_nodes[curNodeIdx].reg_items_queued_bitmap |=
					((uint64_t)1 << ((uint64_t)reg_index));
			curNodeIdx++;
		}while(curNodeIdx < TLE9012_NODES_SIZE);
	}
...
void resetRegisterQueued_tle9012(uint8_t nodeID, uint8_t reg_index,
	...
	if(reg_index == MULTI_READ){
		for(uint8_t i = 0; i < multi_read_cfg_reg_idx_list_num; i++){
			resetRegisterQueued_tle9012(nodeID,
					multi_read_cfg_reg_idx_list[i], isBroadcast);
		}
	}
	else if(isBroadcast == 0){
		DEBUGPRINTF_BITMAP_CHANGE("\t\t\tRegister index "
				"%2d of Node index %2d: NOT Queued\r\n", reg_index, nodeID);
		tle9012_nodes[nodeID-1].reg_items_queued_bitmap &=
				~((uint64_t)1 << ((uint64_t)reg_index));
	}
	else{
...
//****************************************************************************
 inline void setRegisterCacheDataByIndex_tle9012(uint8_t nodeID,
		 uint8_t reg_index, uint16_t reg_data)
{
...
void sendWatchdogUpdate(uint8_t nodeID, uint8_t isBroadcast)
{
    if (isEnabled()) {
		...
		// Refresh interpreter until register is ready,
		// timeout occurred or the loop was executed to often (last resort)
		...
		for(uint32_t i = 0; i<UINT32_MAX; i++){
			...
			if(TIMER_COUNTER_1MS > (startTime + TIMEOUT_COMMAND+50)){
				DEBUGPRINTF("[ERROR]: sendWatchdogUpdate reply timeout "
						"after %ld ms (%ld loops) on Node %d \r\n",
			...
			if(checkRegisterReady_tle9012(nodeID, WDOG_CNT, isBroadcast)){
				DEBUGPRINTF_WATCHDOG("sendWatchdogUpdate reply received "
						"after %ld ms (took %ld ms and %ld loops) on Node %d \r\n",
						TIMER_COUNTER_1MS-timeLastWatchdogRefresh,
						TIMER_COUNTER_1MS - startTime, i, nodeID);
...
//
//***************************************************************************
void setBits_tle9012(uint8_t nodeID, uint8_t reg_index,
		uint16_t bitmask, uint8_t blocking, uint8_t isBroadcast)
{
	if (isEnabled()) {
...
//
//***************************************************************************
void clearBits_tle9012(uint8_t nodeID, uint8_t reg_index,
		uint16_t bitmask, uint8_t blocking, uint8_t isBroadcast)
{
    if(isEnabled()){
    	// Get current register data
