...
//****************************************************************************
// De/activate performance debug messages of interpreter system
#define DEBUGPRINTF_INTERP_PERF(p_frm, ...)	//DEBUGPRINTF(p_frm, ##__VA_ARGS__)
...
uint16_t receive_index = 0; 	// The index of the last byte in the receive ring buffer,
								// new bytes will be added after this index
uint16_t interpreter_index = 0; // The index of the last processed byte in the
								// receive ring buffer, will parse the one after this next
...
uint16_t getInterpreterIndexDifference() {
	...
	// If the receive index is higher than the interpret index
	// a normal difference can be applied
	else if (receive_index > interpreter_index) {
		//DEBUGPRINTF("Normal\r\n");
		return (receive_index - interpreter_index);
	}
	// If the receive index is lower than the interpret index
	// the leap over must be accounted for
	else if (receive_index < interpreter_index) {
		//DEBUGPRINTF("Over\r\n");
...
//****************************************************************************
// calcPackageCRC8
// Calculates the crc8 sum direct in rx_ring_buffer at the given start,
...
void dumpReceiveBuffer() {
	DEBUGPRINTF("Buffer dump - receive index %d, interpreter index %d: ",
			receive_index, interpreter_index);
...
// Checks register index node ID and isSuccessful
// to determine what states shall be set.
//****************************************************************************
static inline void setRegistersStateAndData(uint8_t nodeID, uint8_t regIdx,
		uint16_t data, uint8_t isSuccessful) {
	if (regIdx < NUM_REGISTERS) {
		if (isSuccessful == 1) {
			if (nodeID > 0 && nodeID <= TLE9012_NODES_SIZE) {
				...
				DEBUGPRINTF_INFO("\t\t\t\tNodeID %d "
						"detected as Broadcast - write data to every Node\r\n",
						nodeID);
				...
				DEBUGPRINTF_ERROR("\t\t\t\tNodeID %d is 0 "
						"or exceeded size of NodeArray - Ignoring package\r\n",
						nodeID);
			}
		}
		else {
		...
		DEBUGPRINTF_ERROR("\t\t\t\tRegister index %d "
				"exceeded number of registers: Ignore message \r\n", regIdx);
	}
}
...
uint16_t interpretDataPackage(uint16_t package_index) {
	...
	DEBUGPRINTF_INFO("\t\t\t\tID Frame at: index=%d+%d, byte=0x%02X\r\n",
			package_index, PACKAGE_ID_FRAME_IDX,
			getPackageByte(package_index, PACKAGE_ID_FRAME_IDX));
	...
	uint16_t data =
			((uint16_t)getPackageByte(package_index,
					PACKAGE_DATA0_FRAME_IDX) << 8) |
					getPackageByte(package_index, PACKAGE_DATA1_FRAME_IDX);
	...
	if (isWrite) {
		...
		if (reply_frame == 0) {
			...
			DEBUGPRINTF_SUCCESS("\t\t\t\tStore write data: "
					"NodeID=%d, RegAdr=0x%02X, Data=0x%04X, Reply=0x%02X\r\n",
			...
			DEBUGPRINTF_ERROR("\t\t\t\tWrite command reply indicated "
					"- CRC checked register error:  NodeID=%d, "
					"RegAdr=0x%02X, ReplyByte=0x%02X (CRC3calc=0x%02X)\r\n",
					nodeID, regAdr, reply_frame,
					calcCRC3((uint16_t)(reply_frame>>3)));
			//dumpReceiveBuffer();
			...
			// Print debug info about reply frame
			DEBUGPRINTF_ERROR("\t\t\t\tWrite command reply indicated - "
					"Register address for write command invalid: "
					"NodeID=%d, RegAdr=0x%02X, "
					"ReplyByte=0x%02X (CRC3calc=0x%02X)\r\n",
					nodeID, regAdr, reply_frame,
					calcCRC3((uint16_t)(reply_frame>>3)));
			//dumpReceiveBuffer();
			...
			// Print debug info about reply frame
			/*DEBUGPRINTF_ERROR("\t\t\t\tWrite command reply indicated - "
					"Fault in general diagnostics register. "
					"Will ignore error: NodeID=%d, RegAdr=0x%02X, "
					"ReplyByte=0x%02X (CRC3calc=0x%02X)\r\n",
					nodeID, regAdr, reply_frame,
					calcCRC3((uint16_t)(reply_frame>>3)));*/
			...
			/* Mark successful
			 * a replied problem in general diagnostic register
			 * is not recognized as error
			 * (based on information from datasheet
			 * not sure if write command is ok in this case or not TODO)*/
			isSuccessful = 1;
		}
		else {
			// Print debug info about reply frame
			DEBUGPRINTF_ERROR("\t\t\t\tWrite command reply "
					"indicated unknown error: NodeID=%d, RegAdr=0x%02X, "
					"ReplyByte=0x%02X (CRC3calc=0x%02X)\r\n",
					nodeID, regAdr, reply_frame,
					calcCRC3((uint16_t)(reply_frame>>3)));
			//dumpReceiveBuffer();
		...
		// Return size of write command (there are no write packages)
		// to increase to interpreter index
		return SIZE_WRITE_COMMAND;
	}
	else {
		// READ PACKAGE:
		DEBUGPRINTF_SUCCESS("\t\t\t\tStore read data: "
				"NodeID=%d, RegAdr=0x%02X, Data=0x%04X\r\n",
				nodeID, regAdr, data);
		// Store data to shadow register set ready bit and reset error bit
		setRegistersStateAndData(nodeID, regIdx, data, 1);

		// Return size of read package to increase
		// the interpreter index to next package
...
//****************************************************************************
static inline uint8_t checkDataPackageCRC(uint16_t index_crc_start,
		uint8_t crc_data_size) {
	DEBUGPRINTF_INFO("\t\t\tCalc CRC from index %d for "
			"%d bytes = 0x%02X. Compare to 0x%02X at index %d\r\n",
			index_crc_start, crc_data_size,
			calcPackageCRC8(index_crc_start, crc_data_size),
			getPackageByte(index_crc_start, crc_data_size),
			(index_crc_start + crc_data_size) % RX_RING_BUFFER_SIZE);
	return calcPackageCRC8(index_crc_start, crc_data_size) ==
			getPackageByte(index_crc_start, crc_data_size);
}
...
void tle9012_manageInterpreter() {
	...
	DEBUGPRINTF_INFO("\r\nStart manageInterpreter at "
			"interpreter_index=%d and receive_index=%d\r\n",
	...
	// Debug
	if (!(numBytesInBuffer >= SIZE_MINIMAL_RX_SIZE)) {
		DEBUGPRINTF_INFO("\r\n\tSKIPED interpreter loop - bytes "
				"in fifo MIN=%d, IS=%d ---\r\n\r\n",
				SIZE_MINIMAL_RX_SIZE, numBytesInBuffer); }

	// Interpret ring buffer as long as there is the minimum size
	// of any message in buffer or a bigger incomplete message is found
	while (numBytesInBuffer >= SIZE_MINIMAL_RX_SIZE) {
		DEBUGPRINTF_INFO("\r\n\tSTART interpreter loop - bytes "
				"in fifo MIN=%d, IS=%d ---\r\n",
				SIZE_MINIMAL_RX_SIZE, numBytesInBuffer);

		// The interpreter_index points to the last processed byte
		// in order to make parts of the code simpler and faster.
		// But during the interpreter run the actual index of
		// the first new byte is needed.
		// Therefore it is safely incremented here and
		// used throughout the interpreter routine.
		uint16_t next_interpreter_index =
				getInterpreterAbsoluteIndex(interpreter_index, 1);

		// Reset parse state to tryParsing
		iState = I_PARSE_READ_RETURN;

		// TRY COMMAND PARSE:
		// If "Synchronization Frame" is detected at first byte:
		// New data package is assumed -> check if it is by checking crc sum
		if (getPackageByte(next_interpreter_index, 0) == FRAME_SYNC) {
			DEBUGPRINTF_INFO("\t\tAssumed SYNC frame at %d: "
					"Check CRC to check if this is a command\r\n",
			...
			if ((getPackageByte(next_interpreter_index, 1) & ID_FRAME_ISWRITE_MASK) == 0) {
				...
				// If remaining bytes in buffer are below read command size
				// stop this loop and try next time
				if (numBytesInBuffer < SIZE_READ_COMMAND) {
					DEBUGPRINTF_INCOMPLETE("\t\t\t\tMessage read com incomplete "
							"MIN=%d IS=%d (LAST=0x%02X)\r\n",
							SIZE_READ_COMMAND, numBytesInBuffer,
							getPackageByte(receive_index, 0));
					iState = I_PACKET_INCOMPLETE;
					// Stop loop and hope that next time the whole message is in buffer
					break;
				}
				// Else if CRC8 of this and next 2 bytes
				// (sync, nodeID, regAdr) match with 4th byte (CRC8 byte)
				else if (checkDataPackageCRC(next_interpreter_index, 3)) {
					// Set interpreter index to after this command
					// the replies will be read in "Reply package is assumed"
					incrementInterpreterIndexBy(&interpreter_index, SIZE_READ_COMMAND);
					next_interpreter_index =
							getInterpreterAbsoluteIndex(interpreter_index, 1);
					DEBUGPRINTF_INFO("\t\t\t\tCRC matches = "
							"read command confirmed - "
							"moving interpreter index by %d to %d and try parse\r\n",
							SIZE_READ_COMMAND, interpreter_index);
					// State is still tryParsing
					// next section, try reply parse will become active
				}
				// Else: If the CRC is wrong there is either a invalid package or
				// a data package with a device id similar to the sync frame.
				// In this case the next section will try to parse it
				// as data package because the state still is on I_PARSE_READ_RETURN.
				else {
					DEBUGPRINTF_CRC_FAIL("\t\t\t\tCRC does not match - "
							"must be a data package or a invalid command \r\n");
				}
			}
			// Else write detected (isWrite bit is set)
			else {
				DEBUGPRINTF_INFO("\t\t\tWrite command?\r\n");
				// If remaining bytes in buffer are below write command size
				// stop this loop and try next time
				if (numBytesInBuffer < SIZE_WRITE_COMMAND) {
					DEBUGPRINTF_INCOMPLETE("\t\t\t\tMessage write com "
							"incomplete MIN=%d IS=%d (LAST=0x%02X)\r\n",
							SIZE_WRITE_COMMAND, numBytesInBuffer,
							getPackageByte(receive_index, 0));
					iState = I_PACKET_INCOMPLETE;
					// Stop loop and hope that next time the whole message is in buffer
					break;
				}
				// Else if CRC8 of this and next 4 bytes
				// (sync, nodeID, regAdr, data1, data0) match with 6th byte (CRC8 byte)
				else if (checkDataPackageCRC(next_interpreter_index, 5)) {
					DEBUGPRINTF_INFO("\t\t\t\tCRC matches = "
							"write command confirmed - parse package "
							"and increment index\r\n");
					uint8_t package_size =
							interpretDataPackage(interpreter_index + 2);
					// Parse was successful, no further processing
					// of this data needed - set parse state to: I_PARSE_SUCCESS
					if (package_size > 0) {
						iState = I_PARSE_SUCCESS;
						// Set interpreter index to after this command
						incrementInterpreterIndexBy(&interpreter_index,
								package_size);
						DEBUGPRINTF_SUCCESS("\t\t\t\tParse successful, "
								"moving interpreter index by %d to %d (0x%02X)\r\n",
								package_size, interpreter_index,
								getPackageByte(interpreter_index, 0));
					}
					else {
				...
				// Else: If the CRC is wrong this must be a invalid package.
					...
					DEBUGPRINTF_CRC_FAIL("\t\t\t\tCRC does not match - "
							"must be a invalid command \r\n");
					// TODO: Test if code works if the state is set to
					// failed parse here. Should skip tryParsing
				}
			}
		}

		// TRY REPLY PARSE: If - Reply package is assumed:
		// If Package parse state is not successParse yet and
		// CRC8 of this and next 3 bytes (nodeID, regAdr, data0, data1)
		// match with 5th byte (CRC8 byte)
		if (iState == I_PARSE_READ_RETURN) {
			...
			if (numBytesInBuffer < SIZE_READ_PACKAGE) {
				DEBUGPRINTF_INCOMPLETE("\t\t\tMessage incomplete "
						"MIN=%d IS=%d (LAST=0x%02X)\r\n",
						SIZE_READ_PACKAGE, numBytesInBuffer,
						getPackageByte(receive_index, 0));
				iState = I_PACKET_INCOMPLETE;
				// Stop loop and hope that next time the whole message is in buffer
				break;
			}
			// If CRC8 of this and next 3 bytes
			// (nodeID, regAdr, data0, data1) match with 5th byte (CRC8 byte)
			else if (checkDataPackageCRC(next_interpreter_index, 4)) {
				DEBUGPRINTF_INFO("\t\t\tCRC matches - "
						"parse package and increment index \r\n");
				uint8_t package_size =
						interpretDataPackage(next_interpreter_index);
				// Parse was successful, no further processing of this data needed
				// set parse state to: I_PARSE_SUCCESS
				if (package_size > 0) {
					iState = I_PARSE_SUCCESS;
					// Set interpreter index to after this command
			...
			// Else: If the CRC is still wrong the package is invalid.
			// In this case the next section will discard all bytes
			// until the sync frame is found.
			else {
		...
		if (iState == I_PARSE_FAILED) {
			...
			//// Refresh number of bytes in buffer
			//numBytesInBuffer = getInterpreterIndexDifference();
			//// Increment interpreter index until "Synchronization Frame" is found or
			//// end of buffer is reached. This ignores all broken data
			/*for(uint16_t i = 0; i <= numBytesInBuffer; i++){
				// If synchronization frame is found at any point
				// except the first (could be initial SYNC frame), stop
			...
			// Note: The above loop cannot handle additional bytes between
			// read responses or crc errors in responses.
			// However it would allow for faster processing
