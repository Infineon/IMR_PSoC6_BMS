...
#pragma once
	...
	#if defined (CY8C6245)
		// Target definition for PSoC 6 CY8C6245 on IMR BMS
		// set in Device Configurator on Modus Toolbox
		// 		APP				Name					Comment
		...
		// A delay function must be implemented externally!
		// redefinition needed to allow for multiple targets
		#define DELAY_MS(ms) printf("Delay %dms", ms)
		...
		static inline uint8_t isEnabled(void) {
			printf("isEnabled");
			// Always assume device to be enabled if
			// the supply pins of the TLE9012 are not controlable in hardware
			return 1;
		}
