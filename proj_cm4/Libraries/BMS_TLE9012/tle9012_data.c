 ...
 *
 * lookupTable 		-> 	Pointer to an array of coord_t,
 * 						where each line represents a data point {x,y}.
 * 						X values must be sorted lowest to highest
 * lookupTableSize	-> 	Size of the array of coord_t
 * x				-> 	The x value to be converted to y value
 *
 * Returns a y value for an x based on an lookup table and
 * linear approximation between nearest points
 *
 * Based on https://stackoverflow.com/questions/7091294/
 * how-to-build-a-lookup-table-in-c-sdcc-compiler-with-linear-interpolation
 ****************************************************************************/
double interpLookupTable_tle9012(const coord_t* lookupTable,
		uint16_t lookupTableSize, double x)
{
	// Go thru each line of lookup table until the x value is bigger
	// than current but lower than next line
	for(uint16_t i = 0; i < lookupTableSize-1; i++ ) {
        if ( lookupTable[i].x <= x && lookupTable[i+1].x >= x ) {
            ...
            return lookupTable[i].y +
            		( lookupTable[i+1].y - lookupTable[i].y ) * diffx / diffn;
        }
    }
...
/****************************************************************************
 * interpLookupTableReverse_tle9012
 *
 * lookupTable 		-> 	Pointer to an array of coord_t,
 * 						where each line represents a data point {x,y}.
 * 						X values must be sorted lowest to highest
 * lookupTableSize	-> 	Size of the array of coord_t
 * y				-> 	The y value to be converted to x value
 *
 * Returns an x value for a y based on an lookup table and
 * linear approximation between nearest points
 *
 * Based on https://stackoverflow.com/questions/7091294/
 * how-to-build-a-lookup-table-in-c-sdcc-compiler-with-linear-interpolation
 ****************************************************************************/
double interpLookupTableReverse_tle9012(const coord_t* lookupTable,
		uint16_t lookupTableSize, double y)
{
	// Go thru each line of lookup table until the x value is bigger
	// than current but lower than next line
	for(uint16_t i = lookupTableSize-1; i > 0; i-- ) {
        if ( lookupTable[i].y <= y && lookupTable[i-1].y >= y ) {
            ...
            return lookupTable[i].x +
            		( lookupTable[i-1].x - lookupTable[i].x ) * diffy / diffn;
        }
    }
    return 0; // Not in Range
}
