...
/* While the following lines make things a lot easier like
 * automatically compiling the code for the target you are compiling for
...
#pragma once
	...
	#if defined (CY8C6245)
		// Target definition for PSoC 6 CY8C6245 on IMR BMS set in
		// Device Configurator on Modus Toolbox
		// 		APP				Name					Comment
		//		SPI-3.0			mSPI					SCB5 master set to 3125kHz
		//												and SCLK mode ....TODO
		// 		PIN-3.0 		D_2ED4820_ENABLE		Enable pin APP
		// 		PIN-3.0	 		D_2ED4820_SAVE_STATE_EN	Save state enable pin APP
		// 					 	D_2ED4820_INT			Interrupt pin APP
		//												(connected to 2ED INT pin)
		// MTB includes
		//#include "cy_pdl.h"
		#include "cyhal.h"
		#include "cycfg.h"
		...
		// redefinition needed to allow for multiple targets
		#define DEBUGPRINTF(p_frm, ...) printf(p_frm, ##__VA_ARGS__)

		// A delay function must be implemented externally!
		//extern void delay_ms(uint32_t ms);
		// redefinition needed to allow for multiple targets
		#define DELAY_MS(ms) cyhal_system_delay_ms(ms)
		...
		static inline void enable_interruptPin(void) {
			//cy_rslt_t rslt;
			/*rslt = cyhal_gpio_init(D_2ED4820_INT,
					CYHAL_GPIO_DIR_INPUT, CYHAL_GPIO_DRIVE_NONE, CYBSP_BTN_OFF);*/
			...
			//cyhal_gpio_register_callback(D_2ED4820_INT,
			//		&gpio_2ED4820_callback_data);
			...
			// Set enable of interrupt pin to true
			// NOTE: The pin and the interrupt must be initialized
			// before this is executed!
			// Only nedded where disable_interruptPin() was used e.g. at init.
			// Enable rising edge interrupt event
			// with interrupt priority set to 3
			cyhal_gpio_enable_event(D_2ED4820_INT, CYHAL_GPIO_IRQ_RISE,
					CYHAL_ISR_PRIORITY_DEFAULT, true);
		}

		static inline void disable_interruptPin(void) {
			//PIN_INTERRUPT_SetEdgeSensitivity(&PIN_INT_DRIVER,
			//		PIN_INTERRUPT_EDGE_NONE);
			//cyhal_gpio_register_callback(D_2ED4820_INT, NULL);
			// Disable rising edge interrupt event
			// with interrupt priority set to 3
			cyhal_gpio_enable_event(D_2ED4820_INT, CYHAL_GPIO_IRQ_RISE,
			...
			*data = 0;
			...
			if(isEnabled())
			{
				if(adr >=0 && adr <= 10)
				{
					...
					if(result > 0)
						DEBUGPRINTF("[ERROR]: Can't read register - "
								"cyhal_spi_transfer failed");
					*data = rx_buf[1];
				}
				else
				{
		...
		static inline void spiWrite(uint8_t adr, uint8_t data)
		{
			...
			if(isEnabled())
			{
				...
				if(adr >= 0 && adr <= 10)
				{
					...
					if(result > 0)
						DEBUGPRINTF("[ERROR]: Can't write register - "
								"cyhal_spi_send failed %i %i", tx_buf[0], tx_buf[1]);
				}
			}
			else
			{
		...
		/*static inline void spi_transmit(uint8_t data) {
			...
			while (XMC_SPI_CH_GetStatusFlag(SPI_MASTER_0.channel) &
		...
		/*static inline uint8_t spi_receive(uint8_t data) {
			...
			ReadData = 42;
			SPI_MASTER_Receive(&SPI_MASTER_0, &ReadData, sizeof(ReadData));
			...
			// 		APP				Name					Comment
			//		SPI_MASTER		SPI_DRIVER				Main SPI_MASTER APP.
			//												Config: Full_Duplex,
			//												bus speed = 4MHz,
			//												TX/RX mode = direct,
			//												number of SS lines = 1,
			//												word = 8, Frame = 16,
			//												leading/trailing delay = 1,
			//												enable frame end mode,
			//												Mode: MSB first,
			//												[low if inactive,
			//												 TX on rising, RX on falling],
			//												FIFO enabled 16bit
			// 		DIGITAL_IO  	IO_DRIVER_ENABLE		Enable pin APP
			// 		DIGITAL_IO 		IO_DRIVER_SAVESTATEEN	Save state enable pin APP
			// 		PIN_INTERRUPT 	PIN_INT_DRIVER			Interrupt pin APP
			//												(connected to 2ED INT pin)
			...
			extern void debugPrintf(const char *p_frm, ...);
			// redefinition needed to allow for multiple targets
			#define DEBUGPRINTF(p_frm, ...) debugPrintf(p_frm, ##__VA_ARGS__)
