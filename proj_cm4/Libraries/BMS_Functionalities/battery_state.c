/*
 * battery_state.c
 * 		Abstraction off all battery state related functions
 * 		that need to be accessed from main.
 * 		The implemented algorithm is a simple implementation of
 * 		Coulomb counting based on code from P. Temsamani.
 * 		To implement other algorithms rewrite init_battery_state()
 * 		to be executed at the start of the system,
...
float ocv_curve[OCV_POINTS][2] = {
	...
	// The last 3 Points are manually adapted because slope is steep at this point
	{99.0  , 4.15},		// -> 3495 mAh
	{99.4  , 4.18},		// ->
	{100.0 , 4.20},		// -> 3500 mAh
...
/******************************************************************
 *	init_battery_state
 *	Used at the startup to initialize the SoC and SoH estimation
 *	capacity_rated 	... in As.
 *		The usable capacity of the connected battery
 *		(between over- and under-voltage condition)
 *	voltage_batt	... in V.
 *		The current voltage of the whole battery
 *		(measured over the battery)
 *	numBB           ... The number of cells in he battery
 ******************************************************************/
float init_battery_state(battery_state_struct* bss, float capacity_rated,
		float voltage_batt, uint16_t numBB){
	...
	float initalSoC_fromOCV =
			get_initial_SoC(bss->voltage_init, numBB, BATTERY_STATE_MINMAX_TOL);
	if((isMemoryValid == 1) &&
			((bss->voltage_init/(float)numBB) > SOC_UPPER_TH_USE_LAST_VALUE ||
			 (bss->voltage_init/numBB) < SOC_LOWER_TH_USE_LAST_VALUE) &&
			 (fabs(initalSoC_fromOCV-memoryLastStoredSoC) < SOC_LAST_TO_OCV_MAX_DIFF)){
		bss->SoC_t0 = (float)memoryLastStoredSoC;
		PRINTF_BATTERY_STATE("\t Using SoC = %.2f percent "
				"from FRAM (cell voltage %.2f was below %.2f or over %.2f)\r\n",
				bss->SoC_t0, bss->voltage_init/(float)numBB,
				SOC_LOWER_TH_USE_LAST_VALUE, SOC_UPPER_TH_USE_LAST_VALUE);
	}
	else{
		bss->SoC_t0 = initalSoC_fromOCV;
		PRINTF_BATTERY_STATE("\t Using SoC = %.2f percent from OCV curve "
				"(last SoC in FRAM invalid (valid=%d), "
				"cell voltage %.2f between %.2f and %.2f, "
				"or difference of stored %d to ocv %.0f SoC is higher than %.0f)\r\n",
				bss->SoC_t0, isMemoryValid, bss->voltage_init/numBB,
				SOC_LOWER_TH_USE_LAST_VALUE, SOC_UPPER_TH_USE_LAST_VALUE,
				memoryLastStoredSoC, initalSoC_fromOCV, SOC_LAST_TO_OCV_MAX_DIFF);
	}
	...
	if(isMemoryValid == 1){
		...
		bss->SoH_t0 = SOH_INIT;
		PRINTF_BATTERY_STATE("\t Using SoH = %.2f percent (new) "
				"because last SoH in FRAM not valid %d\r\n",
	...
	// SoH estimation:
	// If capacity_range_SoH is not predefined,
	// approximate it by upper/lower threshold and OCV curve
	if(bss->capacity_range_SoH == 0)
		bss->capacity_range_SoH = capacity_rated / 100 *
								  (SOH_UPPER_THRESHOLD-SOH_LOWER_THRESHOLD);

	// SoH estimation: Estimate voltage at upper and lower threshold
	/* Note: With get_OCV_voltage() the thresholds will be the correlation
	 * to the not loaded OCV curve. In order to get best results
	 * it would be better to use the OCV curve for the charge current
	 * and temperature at which the system is charged!
	 * To compensate this effect the thresholds are set in the region
	 * where the curve is almost linear, and a bit closer together than needed,
	 * to allow for shift up and down.
	 */
...
// Check and manage the state of SoC and SoH calculation,
// this can run every cycle
//****************************************************************************
void manage_battery_state(){
	...
	if(statusBMS.current >= 0.0){ // statusBMS.bms_state == BMS_DISCHARGING || statusBMS.bms_state == BMS_IDLE
		statusBMS.SoC = manageDischarge(&batteryState, setupBMS.BatteryRatedCapacity,
				statusBMS.voltage, (float)(setupBMS.LimitUnderVoltage)/1000.0, ETA_D);
	}
	else if(statusBMS.current < 0.0){ // statusBMS.bms_state == BMS_CHARGING
		statusBMS.SoC = manageCharge(&batteryState, setupBMS.BatteryRatedCapacity,
				statusBMS.voltage, (float)(setupBMS.LimitOverVoltage)/1000.0, ETA_C);
	}
	statusBMS.SoH = estimate_SoH(&batteryState, (battery_states)statusBMS.bms_state,
			(float)setupBMS.BatteryRatedCapacity, ((float)statusBMS.avgCellVolt_mV)/1000.0f);

	// Fake SoC by mapping voltage range to 100 percent
	/*statusBMS.SoC = mapRangeToScale((float)statusBMS.voltage*1000.0,
			SOC_FAKE_MAP_0_PERCENT_mV, SOC_FAKE_MAP_100_PERCENT_mV, 100.0);*/ // (float)setupBMS.LimitUnderVoltageCell*numBatteryBoards, (float)setupBMS.LimitOverVoltageCell*numBatteryBoards, 100.0);

	/// Calculate remaining runtime.
	// For charging, this shows how long the charging will take,
	// for any other state it shows when the battery will be empty
	...
	//	lastCapacity = batteryState.capacity_t;
		if(statusBMS.bms_state == BMS_CHARGING && statusBMS.current < 0.0){
			...
			batteryState.capacity_usable_remaining =
					(BATTERY_RATED_CAPACITY*(statusBMS.SoH/100.0))-
					(batteryState.capacity_init - batteryState.capacity_t);
			// Limit usable capacity
			if(batteryState.capacity_usable_remaining < 0.0)
				batteryState.capacity_usable_remaining = 0.0;
			// Calculate remaining time based on usable capacity
			batteryState.remaining_runtime_ms =
					(int64_t)(batteryState.capacity_usable_remaining/
							(-1.0*statusBMS.current/1000.0)*1000.0);
		}
		else{
			// Calculate usable capacity
			batteryState.capacity_usable_remaining =
					(batteryState.capacity_init - batteryState.capacity_t)-
					(BATTERY_RATED_CAPACITY*REMAINING_RUNTIME_SOC_PERCENT);
			// Limit usable capacity
			if(batteryState.capacity_usable_remaining < 0.0)
				batteryState.capacity_usable_remaining = 0.0;
			// Calculate remaining time based on usable capacity
			batteryState.remaining_runtime_ms =
					(int64_t)(batteryState.capacity_usable_remaining/
							(statusBMS.current/1000.0)*1000.0);
		}
	...
	// Direct capacity and rt data estimation on interval.
	// This is moved to be executed not on a fixed interval
	// but only whenever a new current measurement is done - prevents counting errors.
...
//****************************************************************************
// update_batteryState_currentMeasured
// This performs the integration of the current to get the passed charge.
...
void update_batteryState_currentMeasured(){
	...
	if(dt >= 1){
		...
		measurement_battery_state(&batteryState,
				(battery_states)statusBMS.bms_state,
				statusBMS.current/1000.0, dt); // current in A
		/*PRINTF_MAIN_DEBUG("Update capacity with %f * %f = %f (time %lf)\r\n",
				statusBMS.current, dt, batteryState.capacity_t,
				(float)(TIMER_COUNTER_1MS - lastBatterySateCalcTimestamp));*/
	}
}

/******************************************************************
 *	measurement_battery_state
 *	Use this at every current measurement event
 *	to sum up capacity changes.
 *	current_t ... in A. Current reading at this time step
 *	dt        ... in s. Time since last measurement
 ******************************************************************/
void measurement_battery_state(battery_state_struct* bss, battery_states state,
		float current_t, uint32_t dt){
	...
	/*printf(" < dt %.3f, I %.3f, +%.3f, C_di %.3f, C %.3f> \r\n",
			dt, current_t, current_t * dt, bss->capacity_t,
			(batteryState.capacity_init - batteryState.capacity_t));*/
}

/******************************************************************
 *	update_time_capacity_change_for_rt_recording
 *	Every time this is called the stored time and
 *	capacity difference used for RT data recording is updated.
 *	For precise but not well formatted data
 *	call this from update_batteryState_currentMeasured()
 *	(accumulated time errors due to 1ms inaccuracy)
 *	For well formatted graphs call this more often
 *	to avoid missing or catching a measurement in  the same time
 *	and having jumpy results
 ******************************************************************/
void update_time_capacity_change_for_rt_recording(){
	...
	if(TIMER_COUNTER_1MS > (lastRtDataTimeCapDiffTimestamp + 10)){
		...
		if(dt >= 1){
			...
			// Perform adding of recorded time and
			// charge only if the change of charge is higher than what can be stored minimum
			if(fabs(dC) >= (1.0/MEMORY_RT_DATA_C_DIF_MULTIPLIER)){
				// RT data recording
				rt_data_time_since_last_record += dt;
				rt_data_charge_since_last_record += dC;

				// Debug message to check if integration and
				// rt data value estimation is correct
				/*printf(" < dt %.3f, I %.3f, +%.3f, C_di %.3f, C %.3f> \r\n",
						dt, current_t, current_t * dt, batteryState->capacity_t,
						(batteryState.capacity_init - batteryState.capacity_t));*/
			}
		}
	}
 ...
 *	Use this at every current measurement event
 *	to sum up capacity changes
 *	state 			... state of the charge direction
 *		(manageDischarge, manageCharge or other)
 *	capacity_rated 	... in As.
 *		The usable capacity of the connected battery
 *		(between over- and under-voltage condition)
 *	avgCellVolt		... in V. Voltage average of all cells
 ******************************************************************/
float estimate_SoH(battery_state_struct* bss, battery_states state,
		float capacity_rated, float avgCellVolt){

	// Note: SoH estimation at discharge is deactivated
	// because this yields different results for different discharge currents.
	// Only use this if discharge current is known and adjusted for
	// (e.g.use different OCV curves)!
	...
	if((state == BATTERY_CHARGING && avgCellVolt <= bss->SoH_lower_threshold_V &&
			statusBMS.current > SOH_INITIAL_CURRENT)){
	...
	if((bss->last_state == BATTERY_CHARGING &&
			avgCellVolt > bss->SoH_upper_threshold_V &&
			bss->init_capacity_SoH_at_thres != 0)){
		...
		if(statusBMS.avgTemp > SOH_MIN_TEMPERATURE &&
				statusBMS.avgTemp < SOH_MAX_TEMPERATURE){
			...
			PRINTF_BATTERY_STATE("Temperature (%.2fdegC) was outside limits "
					"(%.2fdegC to %.2fdegC) for SoH estimation. "
					"Calculated value will not be used!\r\n",
					statusBMS.avgTemp, SOH_MIN_TEMPERATURE, SOH_MAX_TEMPERATURE);
		}
		PRINTF_BATTERY_STATE("*******************************************************************************************\r\n");
		PRINTF_BATTERY_STATE("*******************************************************************************************\r\n");
		PRINTF_BATTERY_STATE("*******************************************************************************************\r\n");
		PRINTF_BATTERY_STATE("Set SoH_t: %.3f (calculated %.3f), capacity_state_delta %.3f, "
				"capacity_range_SoH  %.3f, init_capacity_SoH_at_thres %.3f, "
				"last_state %d \r\n", bss->SoH_t, SoH, bss->capacity_state_delta,
				bss->capacity_range_SoH, bss->init_capacity_SoH_at_thres,
				(uint8_t)bss->last_state );
		PRINTF_BATTERY_STATE("*******************************************************************************************\r\n");
		PRINTF_BATTERY_STATE("*******************************************************************************************\r\n");
		PRINTF_BATTERY_STATE("*******************************************************************************************\r\n");
...
/******************************************************************
 *	manageDischarge
 *	Used to calculate changes of the battery state
 *	while it is discharging
 ******************************************************************/
float manageDischarge(battery_state_struct* bss, float capacity_rated,
		float voltage_batt, float v_min, float eta_d) {
	// Calculate capacity change since last change of charge direction
	if(bss->init_capacity_SoH_at_thres != 0)
		bss->capacity_state_delta =
				bss->init_capacity_SoH_at_thres - (bss->capacity_init - bss->capacity_t);

	/*if (bss->SoC_t <= 1 &&
			100/capacity_rated*bss->capacity_state_delta > SOH_MIN_CAPACITY_DELTA) {
		// Calculate state of health in discharge mode
		bss->SoH_t = ((fabsf(bss->capacity_t) + (capacity_rated - bss->capacity_init)) /
				capacity_rated) * 100;
	}
	else {
 ...
 ******************************************************************/
float manageCharge(battery_state_struct* bss, float capacity_rated,
		float voltage_batt, float v_max, float eta_c) {
	// Calculate capacity change since last change of charge direction
	if(bss->init_capacity_SoH_at_thres != 0)
		bss->capacity_state_delta =
				(bss->capacity_init - bss->capacity_t) - bss->init_capacity_SoH_at_thres;
...
/******************************************************************
 *	get_initial_SoC
 *	Voltage to initial SoC via Open Circuit Voltage OCV estimation
 *	Input:
 *		voltage_batt ... in V.
 *		numCells     ... number of cells
 *		tolerance    ... in V.
 *			Tolerance on how far below/above the cell
 *			is still recognized as 0%/100%
 *	Returns SoC in % or -1.0f
 *		if under-voltage error or -2.0f if over-voltage error
 ******************************************************************/
float get_initial_SoC(float voltage_batt, uint16_t numCells, float tolerance) {
	...
	for(uint8_t i = 1; i < OCV_POINTS; i++){
		if(voltage_cell < ocv_curve[i][1]){
            ...
            /*PRINTF_BATTERY_STATE("LastP:%.2f,%.2f CurrentP:%.2f,%.2f; "
            		"k=%.2f d=%.2f, x=%.2f  \r\n",
...
/******************************************************************
 *	get_OCV_voltage
 *	SoC to voltage via Open Circuit Voltage OCV estimation
 *	Input:
 *		percent ... in % charged
