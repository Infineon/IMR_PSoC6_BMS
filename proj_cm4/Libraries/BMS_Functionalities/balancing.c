...
//****************************************************************************
// determineBalancingThreshold
// Calculate current balancing threshold and return lowest cell voltage
//****************************************************************************
uint16_t determineBalancingThreshold(uint16_t* batteryArray){
	...
	if((TIMER_COUNTER_1MS - lastImbalanceCalcTimestamp) >= BALANCING_REFRESH_TIME){
		...
		measure_movAvgFilter_fast(&avgFilter_Imbalance,
				sortBB[setupBMS.NumCells-1].voltage - sortBB[0].voltage);
		statusBMS.imbalance = (uint16_t)roundf(avgFilter_Imbalance.filterResult);

		// Note for future development:
		// If the system should only balance a certain number B of cells at a time,
		// a slope must be implemented to raise the BalVolt
		// to the cell voltage of the B'est cell in sorted array

		// Debug: Print info
		static uint8_t i = 0;
		if(i % 15 == 0){
			PRINTF_BALANCING("%4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d\r\n",
					sortBB[0].idx, sortBB[1].idx, sortBB[2].idx, sortBB[3].idx,
					sortBB[4].idx, sortBB[5].idx, sortBB[6].idx, sortBB[7].idx,
					sortBB[8].idx, sortBB[9].idx, sortBB[10].idx, sortBB[11].idx);
			PRINTF_BALANCING("%4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d %4d\r\n",
					sortBB[0].voltage, sortBB[1].voltage, sortBB[2].voltage,
					sortBB[3].voltage, sortBB[4].voltage, sortBB[5].voltage,
					sortBB[6].voltage, sortBB[7].voltage, sortBB[8].voltage,
					sortBB[9].voltage, sortBB[10].voltage, sortBB[11].voltage);
	...
	if(statusBMS.bms_state == BMS_CHARGING &&
			statusBMS.imbalance > BALANCING_MIN_MAX_THRESHOLD){
...
void manage_balancing(){
	...
	if(isTLE9012ResultReceivedThisCycle == 1){
		...
		uint16_t lowestCellVoltage =
				determineBalancingThreshold(tle9012_nodes[0].cell_voltages);

		/// BALANCING TLE9012:
		// Balancing is only necessary in charging mode,
		// when the register write isn't already queued and
		// the minimal cell voltage is not below threshold
		if( statusBMS.bms_state == BMS_CHARGING &&
		  ( checkRegisterQueued_tle9012(1, BAL_SETTINGS, 0) == 0) &&
		  ( lowestCellVoltage > setupBMS.LimitUnderVoltageCell))
		{
			// BALANCING ALGORITHM:
			// Mark every cell which voltage is between the highest (BalVolt)
			// and a offset limit (BalLimit)
			uint16_t bal_cell_map_new = 0;
			for(uint8_t i = 0; i < NUM_CELLS; i++){
				if(tle9012_nodes[0].cell_voltages[i] > (setupBMS.BalVolt-setupBMS.BalLimit) &&
						tle9012_nodes[0].cell_voltages[i] > setupBMS.LimitUnderVoltageCell){
		...
		// If system is not charging and balancing settings are not 0 deactivate balancing
		else if ((getRegisterCacheDataByIndex_tle9012(1, BAL_SETTINGS) != 0) |
				((statusBMS.message_map & (uint16)MSG_CAF_BAL_ACTIVE) != 0)){
			...
			if(tle9012_result >= REQUEST_ERROR){
				PRINTF_BALANCING("[ERROR]: Deactivate balancing returned %d\r\n",
						tle9012_result);
			}
			tle9012_result = tle9012_readRegisterByIndex(1, BAL_SETTINGS, 0, 0);
			if(tle9012_result >= REQUEST_ERROR){
				PRINTF_BALANCING("[ERROR]: Read balancing register returned %d\r\n",
						tle9012_result);
			}
		}
	}
