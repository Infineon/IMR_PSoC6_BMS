/*
 * global_management.c
 * Abstraction off all settings and objects
 * that can be accessed from every part of the system.
 *
...
// Debug: set initial values to zero for release build
// just for show case during development
...
battery_state_struct batteryState = {
	...
	.last_state = BATTERY_OTHER,		// initial state
										// (will be changed to charge or
										// discharge at first state change)
	.init_capacity_SoH_at_thres = 0.0,	// initial capacity at state change
										// (will be changed to charge or discharge
										// at first state change)
	.capacity_usable_remaining = 0.0,	// in As. Amount of capacity that is still usable
										// (see REMAINING_RUNTIME_SOC_PERCENT)
	.remaining_runtime_ms = 0,			// in ms. Momentary remaining ON time if
										// the system keeps the currently drawn current
										// till the end.
	.capacity_state_delta = 0.0,		// in As. The capacity change since last state change
	.capacity_range_SoH = BATTERY_SOH_CAPACITY_RANGE,	// in As. Amount of capacity
														// that should change between
														// SOH_UPPER_THRESHOLD and
														// SOH_LOWER_THRESHOLD voltage
														// to get 100% SoH.
														// (If zero, it will be estimated
														// from OCV curve.
														// Write correct value after
														// first estimation with expected
														// current here)
	.SoH_upper_threshold_V = 0.0,		// in V. Will be estimated from OCV curve and
										// SOH_UPPER_THRESHOLD at init_battery_state()
										// in (battery_state.h)
	.SoH_lower_threshold_V = 0.0		// in V. Will be estimated from OCV curve and
										// SOH_LOWER_THRESHOLD at init_battery_state()
										// in (battery_state.h)
};
...
volatile setupBMS_struct setupBMS ={
	...
	.CellPackID = { 0xFF, 0xFF, 0xFF, 0xFF },		// First 4 bytes of the BLE address
													// that must be the same throughout
													// all BLE devices in a pack
	.MeasCalib_Offset_I = 0,			    		// Offset used for current calculation
													// (if calibration is perfect
													// this should not be needed)
	.LimitUnderVoltageCell = LIMIT_CELL_UNDER_VOLTAGE_mV,	// Under-voltage limit for each cell
															// If any cell goes under this
															// the safety switch will be
															// activated (MOSFETs blocking)
	.LimitOverVoltageCell  = LIMIT_CELL_OVER_VOLTAGE_mV,	// Over-voltage limit for each cell
															// If any cell goes over this
															// the safety switch will be
															// activated (MOSFETs blocking)
	.LimitUnderVoltage = LIMIT_SYS_UNDER_VOLTAGE_mV,// Under-voltage limit for the whole battery
													// If the pack voltage goes under this
													// the safety switch will be activated
													// (MOSFETs blocking)
	.LimitOverVoltage  = LIMIT_SYS_OVER_VOLTAGE_mV,	// Over-voltage limit for the whole battery
													// If the pack voltage goes over this
													// the safety switch will be activated
													// (MOSFETs blocking)
	.BalVolt = BALANCING_DEACTIVATED_VALUE,			// Initial balancing voltage
													// (reference voltage => lowest cell V in system)
	.BalLimit = BALANCING_CELL_TH_LIMIT,			// Initial balancing limit
													// (cell starts balancing if the own V is
													// BalLimit over BalTol)
	.BatteryRatedCapacity = BATTERY_RATED_CAPACITY	// Rated Capacity = 2.6Ah *
													// 1Cells parallel * 3600 to get As
};
...
volatile uint8_t errorInterruptDetected_2ED4820 = 0;
// BMS analog front end error interrupt detection
volatile uint8_t errorInterruptDetected_TLE9012 = 0;
...
// The trigger of the analog front-end TLE9012 measurement sets this flag
// and leads the measurement of the current, so that they are done simultaneously
uint8_t TriggerCurrentMeasurement = 1;
...
activeChannelExternelADCStates activeChannelExternalADC = 0;
...
uint32_t rt_data_time_since_last_record = 0;
// THIS IS ONLY UPDATED AFTER EACH CURRENT MEASUREMENT
// (TRIGGERED BY ANALOG FRONTEND MEASUREMENT)!
// If rt data is recorded faster than measurement this might be 0
float rt_data_charge_since_last_record = 0;
...
void measure_movAvgFilter_fast(volatile avgFilter* filter, int32_t newValue){
    ...
    // (current index minus filter interval with roll-over check)
    ...
    // Calculate average (at system init the buffer is empty and
    // the divider must be compensated till buffer is filled the first time)
    if(filter->initFilterCompesation > 0){
        filter->filterResult = filter->avgFilterSum /
        		(float)(filter->avgFilterInterval - filter->initFilterCompesation);
...
// TODO: 	Currently this function can only be used for ADC_I_DRIVER_CONV.
// 			Make coefficients as parameter if this is needed more often
//****************************************************************************
