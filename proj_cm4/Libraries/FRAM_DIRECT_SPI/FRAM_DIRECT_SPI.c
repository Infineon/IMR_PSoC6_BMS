...
* GLOBAL CONSTANTS
******************************************************************************/
// Comment out right side (... //printf(...)) to deactivate debug messages
#define PRINTF_MEMORY_DEBUG(p_frm, ...)		printf(p_frm, ##__VA_ARGS__)

/******************************************************************************
* GLOBAL VARIABLES
******************************************************************************/
// 0 = STATE_SUCCESS, 1 = STATE_ERROR
uint8_t spiReinitSuccessful = STATE_SUCCESS;
...
// The SPI object and the marker which mode is currently set
// must be defined in main and is referenced here
extern cyhal_spi_t mSPI;				// SPI used for safety switch 2ED4820,
										// ADC MCP3465 and Memory CY15B256Q-SXA.
										// Use "extern" if needed in other files
extern cyhal_spi_mode_t mSPI_curMode;	// Marks currently set SPI mode.
										// Needed because SPI devices require
										// different SPI modes and must
										// reinitialize SPI if mode is
...
// 32bit value that counts up every millisecond to measure execution time in terminal
#define TIMER_COUNTER_1MS	Cy_TCPWM_Counter_GetCounter(TIMER_1MS_HW, TIMER_1MS_NUM)
#define UART_GET_CHAR_AND_CHECK	(cyhal_uart_getc(&cy_retarget_io_uart_obj, \
		&receivedCharacter, 1) == CY_RSLT_SUCCESS) //(0UL != Cy_SCB_UART_GetNumInRxFifo(UART_HW))
...
void UserDataPattern(uint8_t *wrArray, uint32_t arraySize,
		uint8_t dataByte, uint8_t dataBytebar);
uint32_t FRAM_Burst_Read_And_Compare(uint8_t* expectedData,
		uint32_t startAddress, uint32_t burstLen, uint8_t* bufferRx);
	...
	if(mSPI_curMode != CYHAL_SPI_MODE_00_MSB){
		...
		if (CY_RSLT_SUCCESS == result) {
			...
			if (CY_RSLT_SUCCESS == result){
				...
				PRINTF_MEMORY_DEBUG("\tFailed to set SPI "
						"MEM Frequency %d\r\n", (uint8_t)result);
			}
		}
		else{
...
uint32_t SPIFRAM_RDID(uint8_t* rdID_9B) {
	...
	if(ENABLE_DEBUG_FRAM){
		PRINTF_MEMORY_DEBUG("\r\n Executed Memory ID Read for %ld bytes",
				readBytes);
		PrintArray("\r\n ID - ", &receiveBuf[0], 9);
	}
...
***************************************************************************/
void FRAM_init(void) {
	/* Before executing this function - Set up internal routing, pins,
	 * and clock-to-peripheral connections */
	//init_cycfg_all();
	 ...
	 * Enable SPI on PDL (do not use for direct SPI access) */
...
/*******************************************************************************
*
* FRAM Functions to directly write to FRAM
...
void FRAM_Write_4Kb(uint8_t OPCODE, uint8_t* wData,
		uint32_t startAddress, uint32_t datasize) {
	uint32_t i;
	uint8_t transmitBuf[132];
...
uint32_t FRAM_Read_4Kb(uint8_t OPCODE, uint8_t* rData,
		uint32_t startAddress, uint32_t datasize) {
	uint32_t i;
	uint32_t success = 0;
	uint8_t debugprint[4];
...
void FRAM_Burst_Write(uint8_t* userData, uint32_t startAddress,
	...
	for (index = 0; index < burstLen / nPageSize; index++) {
		FRAM_WREN();
		FRAM_Write(&txBuf[index * nPageSize],
				(startAddress + index * nPageSize), nPageSize);
	}

	if ((burstLen / nPageSize) < nPageSize) {
		FRAM_WREN();
		FRAM_Write(&txBuf[index * nPageSize],
				(startAddress + index * nPageSize), burstLen % nPageSize);
	}
...
void FRAM_Burst_Write_4Kb(uint8_t* userData, uint32_t startAddress,
	...
	for (index = 0; index < burstLen / nPageSize; index++) {
		...
		if ((startAddress + index * nPageSize) < 0xFF)
			FRAM_Write_4Kb(WRITE, &txBuf[index * nPageSize],
					(startAddress + index * nPageSize), nPageSize);
		else
			FRAM_Write_4Kb(WRITE1, &txBuf[index * nPageSize],
					(startAddress + index * nPageSize), nPageSize);
...
void FRAM_Burst_Read(uint8_t* readData, uint32_t startAddress,
	...
	for (index = 0; index < burstLen / nPageSize; index++)
		FRAM_Read(&readData[index * nPageSize],
				index * nPageSize, nPageSize);
...
void FRAM_Burst_Read_4Kb(uint8_t* readData,
		uint32_t startAddress, uint32_t burstLen) {
	uint32_t index;
	uint32_t nPageSize = 64u; //max 128u
	...
	for (index = 0; index < burstLen / nPageSize; index++) {
		if ((startAddress + index * nPageSize) <= 0xFF)
			FRAM_Read_4Kb(READ, &readData[index * nPageSize],
					(startAddress + index * nPageSize), nPageSize);
		else
			FRAM_Read_4Kb(READ1, &readData[index * nPageSize],
					(startAddress + index * nPageSize), nPageSize);
	}

	if ((index * (burstLen / nPageSize)) < burstLen) {
		if ((startAddress + index * nPageSize) <= 0xFF)
			FRAM_Read_4Kb(READ, &readData[startAddress + index * nPageSize],
					(startAddress + index * nPageSize), burstLen % nPageSize);
		else
			FRAM_Read_4Kb(READ1, &readData[startAddress + index * nPageSize],
					(startAddress + index * nPageSize), burstLen % nPageSize);
	}
...
uint32_t FRAM_Burst_Read_And_Compare(uint8_t* expectedData,
	...
	if ((burstLen / nPageSize) < nPageSize) {
		FRAM_WREN();
		FRAM_Read(&bufferRx[index * nPageSize],
				(startAddress + index * nPageSize), burstLen % nPageSize);
	}
...
// Converts ASCII to Hex and combine the nibbles to make byte
void makeBytes(uint8_t* strPtr, uint8_t* Data, uint32_t* cmdLen)
{
	uint32_t i = 0;
...
/***************************************************************************
* Function Name: FRAM_testTerminal.c
* This function will emulate a test terminal for multiple memory functions
* and print debug messages. Once entered it will never be left
...
int FRAM_testTerminal(void) {
	...
	PRINTF_MEMORY_DEBUG("\r\n ******** 1/4/16/64/256 Kb FRAM "
			"Tester (selected MEM_SIZE = %d Kb) *************\r\n",
			(MEM_SIZE*8)/1024);
	PRINTF_MEMORY_DEBUG("\r\n Send following command codes "
			"with parameters to perform test");
	PRINTF_MEMORY_DEBUG("\r\n\t Examples: ");
	PRINTF_MEMORY_DEBUG("\r\n\t\t 020001AABB => command 0x02, "
			"address 0x0001, byte1 0xAA, byte2 0xBB");
	PRINTF_MEMORY_DEBUG("\r\n\t\t 03000102   => command 0x03, "
			"address 0x0001, read 2 byte");
	...
	PRINTF_MEMORY_DEBUG("\r\n case 0x0A: // Opcode for SPI Write (4Kb) - "
			"Upper half of memory array");
	PRINTF_MEMORY_DEBUG("\r\n case 0x0B: // Opcode for SPI Read (4Kb)  - "
			"Upper half of memory array");
	/*PRINTF_MEMORY_DEBUG("\r\n case 0xA2: // Toggle VDDU2_00 off; !0x00 ON");
	PRINTF_MEMORY_DEBUG("\r\n case 0xB1: // Full memory write with Data "
			"in BB (bit bang ~@2.2 MHz) mode");
	PRINTF_MEMORY_DEBUG("\r\n case 0xDD: // SPI transfer on SI as well SO read, "
			"bit banging - DUT on U1");
	PRINTF_MEMORY_DEBUG("\r\n case 0xDE: // SPI transfer on SI as well SO read, "
			"bit banging - DUT on U2");
	PRINTF_MEMORY_DEBUG("\r\n            // DD or DE, 11 adds 10 sec delay "
			"before access, at the beginning");
	PRINTF_MEMORY_DEBUG("\r\n            // DD or DE, 22 adds 10 sec delay "
			"after the access, at the end");
	PRINTF_MEMORY_DEBUG("\r\n            // DD or DE only no (std) delays");
	PRINTF_MEMORY_DEBUG("\r\n case 0xDF: // SPI transfer on SI as well SO read, "
			"bit banging; partial data byte");*/
	PRINTF_MEMORY_DEBUG("\r\n case 0xF1: // Full memory WRITE and "
			"time measurement ");
	PRINTF_MEMORY_DEBUG("\r\n            // 0x77 0x01 generates random data pattern, "
			"0x77 0x02 generates 00 to FF data pattern,");
	PRINTF_MEMORY_DEBUG("\r\n            // any other is used direct");
	PRINTF_MEMORY_DEBUG("\r\n case 0xF2: // Full memory READ and COMPARE with "
			"expected Data and time measurement");
	PRINTF_MEMORY_DEBUG("\r\n            // Use this with same parameters "
			"as 0xF1 to verify write");
	PRINTF_MEMORY_DEBUG("\r\n            // 0x77 0x02 generates 00 to "
			"FF data pattern any other is used direct");
	PRINTF_MEMORY_DEBUG("\r\n case 0xF0: // Full memory write, "
			"read and compare  (GF Test)");
	PRINTF_MEMORY_DEBUG("\r\n case 0xF4: // Full memory write, "
			"read and compare  (GF Test) 4Kb");
	/*PRINTF_MEMORY_DEBUG("\r\n case 0xF8: // Full memory write in SCB mode, "
			"read and verify in SCB and BB mode");
	PRINTF_MEMORY_DEBUG("\r\n case 0xFA: // Power Cycle Test");
	PRINTF_MEMORY_DEBUG("\r\n case 0xFB: // Full memory write with Data-4Kb");
	PRINTF_MEMORY_DEBUG("\r\n case 0xFE: // Memory Density Check");*/
	PRINTF_MEMORY_DEBUG("\r\n case 0xA0: // Set WP/HOLD HI/LO. "
			"0x00 (L/L); 0x0F(L/H); 0xF0/(H/L); 0xFF (H/H); others (H/H)");
	...
	while (1) {
		...
		while (1){
			...
			if(UART_GET_CHAR_AND_CHECK) {
				//receivedCharacter = Cy_SCB_UART_Get(UART_HW);
				strPtr[j] = receivedCharacter;
				j++;
		...
		switch (Data[0]){
			...
			case 0x02: // Opcode for Write
				if (cmdLen / 2 < 3) {
					PRINTF_MEMORY_DEBUG("\r\n Insufficient Data to "
							"execute write operation - Please enter again\r\n");
					break;
				}
			...
			case 0x03: //Opcode for Read
				...
				if (!SPI_4KB) {
					...
					PrintArray("\r\n SPI FRAM read data "
							"(<=4KB - ADDR WIDTH 1B) -", bufferRx, Data[2]);
				}
				break;
			...
			case 0x0A: // Opcode for SPI Write (4Kb) - Upper half of memory array
				...
				FRAM_WREN();
				PRINTF_MEMORY_DEBUG("\r\n 4Kb SPI FRAM burst write "
						"with opcode 0x0A (upper half)\r\n");
			...
			case 0x0B: // Opcode for SPI Read (4Kb) - Upper half of memory array
				...
				PRINTF_MEMORY_DEBUG("\r\n 4Kb SPI FRAM burst read "
						"with opcode 0x0B (upper half)\r\n");
				mrwAddress = Data[1];
				FRAM_Burst_Read_4Kb(bufferRx, mrwAddress, Data[2]);
			...
			case 0xB1: //Full memory write with Data in BB mode
				PRINTF_MEMORY_DEBUG("\r\n BB not implemented, "
						"see commented out section in FRAM_DIRECT_SPI.c\r\n");
				/*PrintArray("\n\r Full memory write with Data DUT on U1 ",
						&Data[1], 2);
				if ((Data[1] == 0x77) && (Data[2] == 0x01)) {
					PRINTF_MEMORY_DEBUG("\n\r Full memory read and "
							"compare with Data 0x77 0x11 "
							"generates random data pattern");
					RandDataPattern(tmpData, MEM_SIZE);
				}
				else if ((Data[1] == 0x77) && (Data[2] == 0x02)) {
					PRINTF_MEMORY_DEBUG("\n\r Full memory read and "
							"compare with Data 0x77 0x11 "
							"generates 00 to FF data pattern");
			...
			case 0xF1: //Full memory write with Data
				...
				PRINTF_MEMORY_DEBUG("\n\r\t Burst Write took %ldms",
						TIMER_COUNTER_1MS-timeBefore);
				//SPI_BB_Burst_Write (tmpData, 0u, MEM_SIZE);
				break;

			case 0xF2: //Full memory read and compare with expected Data

				PrintArray("\n\r Full memory read and compare with Data ",
						&Data[1], 2);
				if ((Data[1] == 0x77) && (Data[2] == 0x01))	{
					PRINTF_MEMORY_DEBUG("\n\r\t 0x77 0x01 "
							"generates random data pattern");
				...
				uint32_t readTimeMs =
						FRAM_Burst_Read_And_Compare(tmpData, 0u,
								MEM_SIZE, bufferRx);
				PRINTF_MEMORY_DEBUG("\n\r\t Burst Read took %ldms",
						readTimeMs);
				break;
			...
			case 0xFC:
				//Full (4Kb) memory read and compare with expected Data
				PrintArray("\n\r Full memory read and compare with Data ",
						&Data[1], 2);
				UserDataPattern(tmpData, MEM_SIZE, Data[1], Data[2]);
