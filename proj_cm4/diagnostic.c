/*
 * diagnostic.c - Abstraction of the diagnostic system, see doSystemDiagnostic for details
 *
...
void resetAndSetErrorsLatchedTLE9012(){
	...
	if ( ((currentErrorBitmap    &         ERR_CAF_PS_SLEEP) == 0) && (TIMER_COUNTER_1MS > (time_last_SYS_PS_SLEEP + TLE9012_COMMON_ERR_LATCH_TIME))){
	...
	if ( ((currentErrorBitmap    &         ERR_CAF_ADC_ERR) == 0) && (TIMER_COUNTER_1MS > (time_last_SYS_ADC_ERR + TLE9012_COMMON_ERR_LATCH_TIME))){
	...
	if ( ((currentErrorBitmap    &         ERR_CAF_OL_ERR) == 0) && (TIMER_COUNTER_1MS > (time_last_SYS_OL_ERR + TLE9012_COMMON_ERR_LATCH_TIME))){
	...
	if ( ((currentErrorBitmap    &         ERR_CAF_INT_IC_ERR) == 0) && (TIMER_COUNTER_1MS > (time_last_INT_IC_ERR + TLE9012_COMMON_ERR_LATCH_TIME))){
	...
	if ( ((currentErrorBitmap    &         ERR_CAF_REG_CRC_ERR) == 0) && (TIMER_COUNTER_1MS > (time_last_REG_CRC_ERR + TLE9012_COMMON_ERR_LATCH_TIME))){
	...
	if ( ((currentErrorBitmap    &         ERR_CAF_EXT_T_ERR) == 0) && (TIMER_COUNTER_1MS > (time_last_EXT_T_ERR + TLE9012_OVERTMP_ERR_LATCH_TIME))){
	...
	if ( ((currentErrorBitmap    &         ERR_CAF_INT_OT) == 0) && (TIMER_COUNTER_1MS > (time_last_INT_OT + TLE9012_OVERTMP_ERR_LATCH_TIME))){
	...
	if ( ((currentErrorBitmap    &         ERR_CAF_CELL_UV) == 0) && (TIMER_COUNTER_1MS > (time_last_CELL_UV + TLE9012_COMMON_ERR_LATCH_TIME))){
	...
	if ( ((currentErrorBitmap    &         ERR_CAF_CELL_OV) == 0) && (TIMER_COUNTER_1MS > (time_last_CELL_OV + TLE9012_COMMON_ERR_LATCH_TIME))){
	...
	if ( ((currentErrorBitmap    &         ERR_CAF_BAL_UC) == 0) && (TIMER_COUNTER_1MS > (time_last_BAL_UC + TLE9012_COMMON_ERR_LATCH_TIME))){
	...
	if ( ((currentErrorBitmap    &         ERR_CAF_BAL_OC) == 0) && (TIMER_COUNTER_1MS > (time_last_BAL_OC + TLE9012_COMMON_ERR_LATCH_TIME))){
	...
	else if (TIMER_COUNTER_1MS > (time_last_TLE9012_QUEUE_FAILED + TLE9012_COMMON_ERR_LATCH_TIME)){
	...
	else if (TIMER_COUNTER_1MS > (time_last_TLE9012_INTERRUPT + TLE9012_COMMON_ERR_LATCH_TIME)){
...
void diagnosticSafetySwitch(){
	...
	// If an 2ED4820 error is detected (error interrupt or internal error detected) try to reset
	if( error_map_safety_switch != 0 || errorInterruptDetected_2ED4820 == 1 || cyhal_gpio_read(D_2ED4820_INT) == 1){
		if(errorInterruptDetected_2ED4820 == 1 || cyhal_gpio_read(D_2ED4820_INT) == 1){
			// Mark error
			setErrorBit(ERR_SSW_INTERRUPT);
			PRINTF_DIAGNOSTIC("[ERROR]: Safety switch - Interrupt %d, Pin %d, ErrorMap %ld\r\n", errorInterruptDetected_2ED4820, cyhal_gpio_read(D_2ED4820_INT), error_map_safety_switch);
		}
...
//****************************************************************************
// diagnosticTLE9012 - Reset and set errors of TLE9012. Handles the read and reset of diagnostic registers of the analog frontend TLE9012
	...
	if(checkRegisterQueued_tle9012(1, GEN_DIAG, 0) == 0){
		if(diagnostic_state == DIAGNOSTIC_EVAL_CLEAR){
			...
			if(checkRegisterError_tle9012(1, GEN_DIAG, 0) == 0){
				// The error bits of the analog frontend TLE9012 need to be latched for some time because the result might wrongly be 0 when clear was executed but round robin cycle did not run yet.
				...
				if((data_gen_diag & GEN_DIAG_RESETABLE_FLAGS) != 0){
					...
					/// If an error that is cell specific was found store and read its cell map (the cell map will effectively be right in the next turn)
					// Open load
					if(statusBMS.error_map & ERR_CAF_OL_ERR){
						statusBMS.ol_map |= getRegisterCacheDataByIndex_tle9012(1, DIAG_OL);
						TLE9012_DETECT_COM_ERROR( tle9012_readRegisterByIndex(1, DIAG_OL, 1, 0), tle9012_readRegisterByIndex_DIAG_OL , SUCCESS_FUNCTION_NONE);
					}
					else{
						statusBMS.ol_map = 0;
					}
					...
					if(statusBMS.error_map & ERR_CAF_CELL_UV){
						...
						TLE9012_DETECT_COM_ERROR( tle9012_readRegisterByIndex(1, CELL_UV, 1, 0), tle9012_readRegisterByIndex_CELL_UV , SUCCESS_FUNCTION_NONE);
					}
					else{
						statusBMS.uv_map = 0;
					}
					...
					if(statusBMS.error_map & ERR_CAF_CELL_OV){
						...
						TLE9012_DETECT_COM_ERROR( tle9012_readRegisterByIndex(1, CELL_OV, 1, 0), tle9012_readRegisterByIndex_CELL_OV , SUCCESS_FUNCTION_NONE);
					}
					else{
						statusBMS.ov_map = 0;
					}
					...
					if(statusBMS.error_map & ERR_CAF_BAL_UC){
						statusBMS.bal_uc_map |= getRegisterCacheDataByIndex_tle9012(1, BAL_DIAG_UC);
						TLE9012_DETECT_COM_ERROR( tle9012_readRegisterByIndex(1, BAL_DIAG_UC, 1, 0), tle9012_readRegisterByIndex_BAL_DIAG_UC , SUCCESS_FUNCTION_NONE);
					}
					else{
					...
					if(statusBMS.error_map & ERR_CAF_BAL_OC){
						statusBMS.bal_oc_map |= getRegisterCacheDataByIndex_tle9012(1, BAL_DIAG_OC);
						TLE9012_DETECT_COM_ERROR( tle9012_readRegisterByIndex(1, BAL_DIAG_OC, 1, 0), tle9012_readRegisterByIndex_BAL_DIAG_OC , SUCCESS_FUNCTION_NONE);
					}
					else{
					...
					//if(errorInterruptDetected_TLE9012 == 1 || cyhal_gpio_read(TLE9012_ERR) == 1){
					//	PRINTF_DIAGNOSTIC("[ERROR]: errorInterruptDetected_TLE9012 detected %d, pin state was %d, GEN_DIAG ", errorInterruptDetected_TLE9012, cyhal_gpio_read(TLE9012_ERR));
					...
					if(tle9012_result >= REQUEST_ERROR){ PRINTF_DIAGNOSTIC("[ERROR]: tle9012_clearStatusFlags returned %d\r\n", tle9012_result);}
				}
			}
			// Change state to read
			diagnostic_state = DIAGNOSTIC_READ;
		}
		else if((diagnostic_state == DIAGNOSTIC_READ && measureState == MEASUREMENT_READ_RESULT) || errorInterruptDetected_TLE9012){
			//PRINTF_DIAGNOSTIC("Read GEN_DIAG\r\n");
			tle9012_result = tle9012_readRegisterByIndex(1, GEN_DIAG, 0, 0); // 0xE001
			if(tle9012_result >= REQUEST_ERROR){ PRINTF_DIAGNOSTIC("[ERROR]: Trigger next GEN_DIAG read failed %d\r\n", tle9012_result);}
...
//****************************************************************************
// diagnosticInternal - Reset and set internal system errors that are not from safety switch driver or analog frontend
//****************************************************************************
void diagnosticInternal(){
	...
	if((statusBMS.current > (float)OVERCURRENT_THRESHOLD_POS_mA) || (statusBMS.current < (float)OVERCURRENT_THRESHOLD_NEG_mA)){
	...
	if(TIMER_COUNTER_1MS > (lastTLE9012IntervalComErrorReduceTimestamp + TLE9012_INTERVAL_COM_ERRORS)){
	...
	// Check if a switch ON veto was received where the other BMS send value above 100 to signal that it has its output active.
	if(receivedCanSwitchOnVetoCount > 0 && receivedCanSwitchOnVetoActiveState == 1 && statusBMS.hotSwapRole != HOTSWAP_TAKER){ // && statusBMS.slotState == SLOT_IN_SYSTEM_2
	...
	// Charge from undervoltage: If system is in a charger and the external load voltage is higher than main voltage and not below minimum - clear undervoltage errors
	if( ((statusBMS.slotState == SLOT_IN_CHARGER) && ((statusBMS.extLoadVoltage+CHARGE_FROM_UV_EXT_OFFSET_V) > statusBMS.voltage)) && (statusBMS.extLoadVoltage > CHARGE_FROM_UV_EXT_MIN_V)){ //
...
// Uses measureState to only manage analog frontend TLE9012 error between measurements
//****************************************************************************
...
//	printBinary(MSG_INT_MASK, UINT32_MAX, 32); PRINTF_DIAGNOSTIC("\r\n");
	...
	// If any errors were found, set state to ERROR ... if state is ERROR and no errors are present set to IDLE ... else check if charge state is below warning limit and blink red LED
	if(statusBMS.error_map != 0){
		...
		if(getMsgBit(MSG_INT_BATTERY_LOW) == 1 && (TIMER_COUNTER_1MS > (batteryLowWarningTimestamp + BATTERY_LOW_WARNING_DELAY_TIME))){
			// Blink red LED with different on and off times. At beginning the LED is witch on (first if), after BLINK_TIME_LED_RED_ON_TIME the LED is kept off (second if) until the cycle time BLINK_TIME_LED_RED has passed
			...
			//PRINTF_DIAGNOSTIC("map %ld, getMSG %d, Bit %d", statusBMS.message_map, getMsgBit(MSG_INT_BATTERY_LOW), MSG_INT_BATTERY_LOW);
			// In normal operation this keeps the timestamp updated. This way the above cause will only trigger if the SoC stays to low for BATTERY_LOW_WARNING_DELAY_TIME
...
//****************************************************************************
// diagnostic_printStatusFlags - Check error and/or warning bitmap and report failures to debug
//****************************************************************************
void diagnostic_printStatusFlags(uint32_t bitmap_error, uint32_t bitmap_warning, uint8_t newLineForEachError, uint8_t onlyPrintIfDataChanged)
{
	...
	if(bitmap_error != 0 && (onlyPrintIfDataChanged == 0 || (onlyPrintIfDataChanged == 1 && last_error_map != bitmap_error))){
	...
	if(bitmap_warning != 0 && (onlyPrintIfDataChanged == 0 || (onlyPrintIfDataChanged == 1 && last_warning_map != bitmap_warning))){
		if(newLineForEachError == 0)
