...
*
*              Note that commonly needed variables are grouped into statusBMS, setupBMS and batteryState.
*              Note that "ram_public.h" and "ipc_def.h" is used for communication between processors and the public RAM area.
*
*******************************************************************************
...
#include "global_management.h"	// Common elements between all components on CM4 core
#include "safety_switch.h"		// Safety switch related functions (init, get/set switch state, etc)
...
#include "MCP3x6x/MCP3x6x_SPI.h"
#include "MCP3x6x/MCP3x6x_Type.h"
...
#define PRINTF_MEASUREMENT(p_frm, ...)				//printf(p_frm, ##__VA_ARGS__)
#define PRINTF_MAIN_RT_GETLINE_DEBUG(p_frm, ...)	//printf(p_frm, ##__VA_ARGS__)
...
cyhal_spi_t mSPI;				// SPI used for safety switch 2ED4820, ADC MCP3465 and Memory CY15B256Q-SXA. Use "extern" if needed in other files
cyhal_spi_mode_t mSPI_curMode;	// Marks currently set SPI mode. Needed because SPI devices require different SPI modes and must reinitialize SPI if mode is not correct for them. Use "extern" if needed in other files
...
// Marker for direct save state set on error interrupt. If this is 0 any error interrupt will activate the save state for a short time to switch system off without main loop.
...
void reset_latchedBitmaps(uint8_t reset_error_map_switch_on, uint8_t reset_error_map_ext_com);
void calc_voltageChangeRate(float loadVoltage, float minLoadVoltage);
uint8_t updatePublicDisplayData(public_core_commands command); // Update public variables from shadow buffer if resource is not used at the moment (non blocking)

// Interrupt routines
void timer_watchdog_interrupt_handler();
static void IRQ_ERR_2ED4820(void* handler_arg, cyhal_gpio_event_t event); // Error pin interrupt from safety switch
static void IRQ_ERR_TLE9012(void* handler_arg, cyhal_gpio_event_t event); // Error pin interrupt from analog frontend


// Test functions
void debugButtonCheck(); // Function extraction of debug code executed when debug button is pressed
...
int main(void)
{
    ...
    // ---- INIT CM4 ---- blocking code allowed (but must finished in reasonable time)
    /* Initialize the device and board peripherals */
    result = cybsp_init() ;
    if (result != CY_RSLT_SUCCESS){ cyhal_gpio_write(BMS_LED3_RD, 1); cyhal_system_delay_ms(500); cyhal_gpio_write(VCC_HOLD, 0); CY_ASSERT(0); }
	...
	// From this point keep the power alive. The green MCU ready LED is turned on later when pin is full on o that system does not shut down after it is released.
	// Doing it here prevents the system from starting up after reset/shutdown etc
	...
	// Init Memory
	//FRAM_testTerminal(); // Enter a testing terminal for the memory. Need to be activated in FRAM_DIRECT_SPI.h
	init_memory();

	// Initialize MCP3465 external ADC and wait till enough measurements are done to ensure correct filtering
    ...
    for (;;)
    {
		...
		// Check if balancing conditions are met, execute balancing algorithm and determine the mapping of which cell needs balancing (actual write performed in processMeasurements())
		manage_balancing();

	    // Receive (when possible) errors and warnings from safety switch, analog frontend TLE9012 and rest of system.
	    // Set bms_state to BMS_ERROR if needed or to BMS_IDLE if error cleared. Controls red LED.
	    doSystemDiagnostic();

		// Control switch state based on bms_state. When no error present determine bms_state charge direction, precharge and minimum restart timer.
		manage_SystemAndSwitchState();

		// Manage data from and to the BMS (CAN, UART debug, etc). Resets latched bitmaps
    	...
    	// Update public variables from shadow buffer if resource is not used at the moment (non blocking)
    	...
    	// Keep software watchdog alive - Increment main loop counter to keep watchdog handler inactive
    	mainLoopCounter++;
...
//****************************************************************************
// timer_watchdog_interrupt_handler - Handler function for the watchdog timer (check if main loop died)
//****************************************************************************
void timer_watchdog_interrupt_handler(){
    // If main loop counter did non change since last time shut down system
    if(isWatchdogActive == 1 && mainLoopCounter == mainLoopCounter_lastWatchdog){
    	PRINTF_MAIN_DEBUG("System watchdog detected no activity for %d ms and will deactivate system (%ld == %ld)\r\n", SOFTWARE_WATCHDOG_TIMEOUT, mainLoopCounter, mainLoopCounter_lastWatchdog);
    ...
    // Kill system if shutdown button is continuously pressed for multiple seconds (this code also runs on CM0 to make sure a shutdown is possible even if one core stops working)
	...
	if(TIMER_COUNTER_1MS > (shutdownButtonNotPressedTimestamp + MAXIMUM_SHUTDOWN_TIME)){
		PRINTF_MAIN_DEBUG("HARD SHUTDOWN TRIGGERED - BUTTON PRESSED FOR LONGER THAN MAXIMUM_SHUTDOWN_TIME\r\n");
		cyhal_gpio_write(VCC_HOLD, 0);
		shutdownSystem();
	}
    ...
    Cy_TCPWM_ClearInterrupt(TIMER_WATCHDOG_HW, TIMER_WATCHDOG_NUM, CY_TCPWM_INT_ON_TC );
}
...
void init_UART_tle9012(){
	...
	if (result_uart_tle == CY_RSLT_SUCCESS){ PRINTF_MAIN_DEBUG("Init UART TLE9012: %d\r\n", (int)result_uart_tle); }
	else{ PRINTF_MAIN_DEBUG("Init UART TLE9012 failed: %d\r\n", (int)result_uart_tle); }
}
...
void init_Timer_RTC(){
	...
	cy_en_tcpwm_status_t result_timer = Cy_TCPWM_Counter_Init(TIMER_1MS_HW, TIMER_1MS_NUM, &TIMER_1MS_config);
	if (CY_TCPWM_SUCCESS != result_timer) {PRINTF_MAIN_DEBUG("Init TIMER_1MS failed: %d\r\n", (int)result_timer);}
	...
	cy_rslt_t result = Cy_SysInt_Init(&timer_watchdog_interrupt_config, timer_watchdog_interrupt_handler);
	if(result != CY_SYSINT_SUCCESS){ PRINTF_MAIN_DEBUG("Interrupt init TIMER_WATCHDOG failed: %d\r\n", (int)result); }
	/* Enable Interrupt */
	NVIC_EnableIRQ(timer_watchdog_interrupt_config.intrSrc);

	// Enable watchdog timer
	cy_en_tcpwm_status_t result_timer_wd = Cy_TCPWM_Counter_Init(TIMER_WATCHDOG_HW, TIMER_WATCHDOG_NUM, &TIMER_WATCHDOG_config);
	if (CY_TCPWM_SUCCESS != result_timer_wd) {PRINTF_MAIN_DEBUG("Init TIMER_WATCHDOG failed: %d\r\n", (int)result_timer_wd);}
	// Enable the initialized counter
	...
	if (0UL != (CY_TCPWM_INT_ON_TC & Cy_TCPWM_GetInterruptMask(TIMER_WATCHDOG_HW, TIMER_WATCHDOG_NUM))) {
		Cy_TCPWM_SetInterrupt(TIMER_WATCHDOG_HW, TIMER_WATCHDOG_NUM, CY_TCPWM_INT_ON_TC);
	}

    // Set the timer period in milliseconds. To count N cycles, period should be set to N-1.
    Cy_TCPWM_Counter_SetPeriod(TIMER_WATCHDOG_HW, TIMER_WATCHDOG_NUM, SOFTWARE_WATCHDOG_TIMEOUT-1 );
...
//****************************************************************************
// init_SPI_peripheral - Startup and setup SPI used for all peripherals except display
//****************************************************************************
void init_SPI_peripheral() {
	cy_rslt_t result;
	// Init hal SPI for 2ED4820 (does not matter for which device it is configured, will reinitialized when needed for each device)
	...
	// Init SPI CS 2ED4820
    result = cyhal_gpio_init(PERI_SPI_CS_2ED4820, CYHAL_GPIO_DIR_OUTPUT, CYHAL_GPIO_DRIVE_STRONG, 1);
	if (CY_RSLT_SUCCESS == result) {
    ...
    result = cyhal_gpio_init(PERI_SPI_CS_MCP3465, CYHAL_GPIO_DIR_OUTPUT, CYHAL_GPIO_DRIVE_STRONG, 1);
	if (CY_RSLT_SUCCESS == result) {
    ...
    result = cyhal_gpio_init(PERI_SPI_CS_CY15B, CYHAL_GPIO_DIR_OUTPUT, CYHAL_GPIO_DRIVE_STRONG, 1);
	if (CY_RSLT_SUCCESS == result) {
	...
	// Init enable pin
    result = cyhal_gpio_init(D_2ED4820_ENABLE, CYHAL_GPIO_DIR_BIDIRECTIONAL, CYHAL_GPIO_DRIVE_STRONG, 0);
	if (CY_RSLT_SUCCESS == result) {
	...
	// Init safestate pin
	result = cyhal_gpio_init(D_2ED4820_SAVE_STATE_EN, CYHAL_GPIO_DIR_BIDIRECTIONAL, CYHAL_GPIO_DRIVE_PULLDOWN, 0);
	if (CY_RSLT_SUCCESS == result) {
...
void init_interrupt_2ED4820(){
	cy_rslt_t rslt;
	rslt = cyhal_gpio_init(D_2ED4820_INT, CYHAL_GPIO_DIR_INPUT, CYHAL_GPIO_DRIVE_PULLDOWN, 0);
    ...
    cyhal_gpio_enable_event(D_2ED4820_INT, CYHAL_GPIO_IRQ_RISE, CYHAL_ISR_PRIORITY_DEFAULT, 1);	// Enable rising edge interrupt event with interrupt priority set to 3
}
...
void init_interrupt_TLE9012(){
	cy_rslt_t rslt;
	rslt = cyhal_gpio_init(TLE9012_ERR, CYHAL_GPIO_DIR_INPUT, CYHAL_GPIO_DRIVE_PULLDOWN, 0);
	if(CY_RSLT_SUCCESS != rslt){
	...
	cyhal_gpio_register_callback(TLE9012_ERR, &gpio_TLE9012_callback_data);		    				// Register callback function - gpio_interrupt_handler and pass the value global_count
    cyhal_gpio_enable_event(TLE9012_ERR, CYHAL_GPIO_IRQ_RISE, CYHAL_ISR_PRIORITY_DEFAULT, true);	// Enable falling edge interrupt event with interrupt priority set to 3
}
...
void init_external_ADC_MCP3465() {
	...
	// Try to read results until wanted number of reads are read, timeout occurred (used for loop to avoid while(true))
	for(uint16_t i = 0; i < UINT16_MAX; i++){
		...
		if(readSuccessfulCount >= ADC_EXT_ZEROING_NUM){
			PRINTF_MAIN_DEBUG("\t\tExternal ADC read %d times for zeroing. Read successful after %ld ms\r\n", readSuccessfulCount, TIMER_COUNTER_1MS-adcZeroingTimestamp);
			return;
		}

		// Stop loop if takes to long
		if(TIMER_COUNTER_1MS > (adcZeroingTimestamp + ADC_EXT_ZEROING_TIMEOUT_TIME)){
			PRINTF_MAIN_DEBUG("[ERROR]: External ADC read %d times for zeroing. Read took too long (Timeout)\r\n", readSuccessfulCount);
			// Mark permanent error (if this flag is set manage_external_ADC_MCP3465 will keep the timeout and therefore the error triggered even if it disappears)
			initExtADCError = 1;
			return;
		}
...
//****************************************************************************
// init_hotSwap - Determine if the BMS shall start normally (alone in system, in charger or not in slot) in HotSwap MAIN mode or in HotSwap TAKER role
//****************************************************************************
void init_hotSwap(){
	...
	while(TIMER_COUNTER_1MS < (hotSwapInitStartTimestamp + HOTSWAP_INIT_SWITCHON_VETO_TIME)){
		...
		if(receivedCanSwitchOnVetoCount != 0){
			...
			switchOnVetoReceivedActiveStateOtherBMS |= receivedCanSwitchOnVetoActiveState; // Used or operator to make variable true even if it was 1 only once
		}
		// Check if a external load voltage was detected (other BMS is supplying)
	...
	PRINTF_MAIN_DEBUG("\tInitial slot state %d stayed constant = %d. External load voltage above threshold = %d. Switch on veto send successful = %d (own SoC %d). Switch on veto received = %d (SoC other BMS = %d)\r\n", hotSwap_initialSlotState, wasSlotStateConstantOnASystem, wasExternalVoltageAboveThreshold, switchOnVetoSendSuccessCounter, targetData[0], wasSwitchOnVetoReceived, switchOnVetoReceivedSocOtherBMS);

	/// Check in which state the BMS should start. The first two condition are used to detect definitive states, the rest detects special states on to of them
	// Shutdown condition - slot changed during init
	if(wasSlotStateConstantOnASystem == 0){
		PRINTF_MAIN_DEBUG("\tState can not be determined - shutdown system. Slot state did not stay constant during hotswap init\r\n");
		statusBMS.hotSwapRole = HOTSWAP_ERROR;
		shutdownSystem();
	}
	// Explicit TAKER condition - Always wait for takeover if external load voltage is detected
	else if(wasExternalVoltageAboveThreshold == 1 && (hotSwap_initialSlotState == SLOT_IN_SYSTEM_1 || hotSwap_initialSlotState == SLOT_IN_SYSTEM_2)){
		PRINTF_MAIN_DEBUG("\tState explicitly TAKER. Slot state stayed constant, but external load voltage was detected\r\n");
		statusBMS.hotSwapRole = HOTSWAP_TAKER;
	}
	// Explicit MAIN/GIVER condition - No switchOnVeto CAN message received and not able to send any (there is no second BMS)
	else if((hotSwap_initialSlotState != SLOT_IN_SYSTEM_1) && (hotSwap_initialSlotState != SLOT_IN_SYSTEM_2)){
		PRINTF_MAIN_DEBUG("\tState explicitly MAIN. Slot state stayed constant and no external load voltage was detected, but system is not in a system slot\r\n");
		statusBMS.hotSwapRole = HOTSWAP_MAIN;
	}
	else if(wasSwitchOnVetoReceived == 0 && switchOnVetoSendSuccessCounter <= 1){
		PRINTF_MAIN_DEBUG("\tState explicitly MAIN. Slot state stayed constant and no external load voltage was detected, but no switch ON veto received and switch ON veto sending was not successful\r\n");
		statusBMS.hotSwapRole = HOTSWAP_MAIN;
	}
	// TAKER condition, second BMS startup - A second BMS is active but did not enable output yet
	else if(switchOnVetoReceivedActiveStateOtherBMS == 1){
		PRINTF_MAIN_DEBUG("\tState second BMS TAKER. Slot state stayed constant, no external load voltage was detected, switch ON veto received, but other BMS sends active flag\r\n");
		statusBMS.hotSwapRole = HOTSWAP_TAKER;
	}
	// Both BMS want to be active - determine hotswap role
	else if(wasSwitchOnVetoReceived == 1 && switchOnVetoSendSuccessCounter > 1){
		if(statusBMS.slotState == SLOT_IN_SYSTEM_1){
			PRINTF_MAIN_DEBUG("\tState MAIN. Slot state stayed constant and no external load voltage was detected, switch ON veto received, switch ON veto sending successful, but BMS is in main slot 1\r\n");
			statusBMS.hotSwapRole = HOTSWAP_MAIN;
		}
		else if(statusBMS.slotState == SLOT_IN_SYSTEM_2){
			PRINTF_MAIN_DEBUG("\tState TAKER. Slot state stayed constant and no external load voltage was detected, switch ON veto received, switch ON veto sending successful, but BMS is in main slot 2\r\n");
			statusBMS.hotSwapRole = HOTSWAP_TAKER;
		}
		else{
		...
		PRINTF_MAIN_DEBUG("\t[ERROR]: HotSwap init encountered not handled condition, make sure that there is no CAN sniffer active (would ack switch on veto messages but does not reply)\r\n");
		statusBMS.hotSwapRole = HOTSWAP_ERROR;
		shutdownSystem();
	}


	// If role allows switch on send a first switch on veto message with value set to max to signal that this BMS will try to switch on
	if(statusBMS.hotSwapRole == HOTSWAP_MAIN){
		...
		// Reset can message counter so that system is in known state (especially switch on veto counter would trigger ERR_INT_SWITCH_ON_VETO_RECEIVED if not reset)
		resetCanMessageCounters();
	}
...
void init_batteryState(){
	...
	statusBMS.SoC = init_battery_state(&batteryState, setupBMS.BatteryRatedCapacity, statusBMS.voltage, setupBMS.NumCells);
	statusBMS.SoH = batteryState.SoH_t0; // For the time being - will later be replaced with a stored SoH value
	PRINTF_MAIN_DEBUG("\tSoC_t0 %.3f, capacity_init %.3fmAh, voltage_init %.3f, SoH_lower_threshold_V %.3f, SoH_upper_threshold_V %.3f, capacity_range_SoH %.3f\r\n", batteryState.SoC_t0, batteryState.capacity_init/3.6, batteryState.voltage_init, batteryState.SoH_lower_threshold_V, batteryState.SoH_upper_threshold_V, batteryState.capacity_range_SoH);
}

//****************************************************************************
// init_analogFrontendTLE9012 - Initialize communication with the analog frontend TLE9012, configure it, read initial data and refresh tle9012 watchdog s it does not turn of before main loop starts
//****************************************************************************
requestResult init_analogFrontendTLE9012(){
	...
	uint16_t ov_thr     = (uint16_t)((((float)setupBMS.LimitOverVoltageCell)/1000.0)/CELL_OVUV_LSB);
	...
	uint16_t uv_thr     = (uint16_t)((((float)(setupBMS.LimitUnderVoltageCell))/1000.0)/CELL_OVUV_LSB);
	...
	statusBMS.avgTemp = (float)(tle9012_nodes[0].temperatures[0] + tle9012_nodes[0].temperatures[3])/2; // Calculate the average temperature between cell temperature sensors
	statusBMS.avgCellVolt_mV = UINT16_MAX;

	// Send a watchdog update because the next steps till the main loop enters could take longer than wake time. NOTE: This is only allowed here because the last command was executed blocking and is is therefore sure that no data can be received during the update (see comment in sendWatchdogUpdate())
...
//****************************************************************************
// processMeasurements - Manages the measurement and conversion of analog frontend (cell voltage, block voltage temperatures and everything in multiread), external ADC (current and load voltage) and internal ADC.
//						 Note that the current measurement is triggered in the same cycle as the analog frontend measurements, as long as they are not slower
//****************************************************************************
void processMeasurements(){
	// Reset flag that notices rest of code that the result are ready
	isTLE9012ResultReceivedThisCycle = 0;

	// TLE9012 analog frontend measurement processing state machine -> Cell/block voltage, temperature, diagnostic, etc (see call of tle9012_writeMultiReadConfig() in init)
	requestResult tle9012_result;
	switch (statusBMS.measurementState) {
		case MEASUREMENT_WAIT_RESULT:
			// Check the MEAS_CTRL control register until the trigger bit is gone (in this case the measurement is finished)

			// If MEAS_CTRL is not queued anymore read is finished
			if(checkRegisterQueued_tle9012(1, MEAS_CTRL, 0) == 0){
				// If the PCVM and BVM Start measurement bit is gone trigger MULTI_READ, else trigger next read of MEAS_CTRL
				if((getRegisterCacheDataByIndex_tle9012(1, MEAS_CTRL) & (MEAS_CTRL__PCVM_START.mask | MEAS_CTRL__BVM_START.mask)) == 0){
					PRINTF_MEASUREMENT("Measurement finished - Queue next multi read\r\n");

					// BALANCING: Write balancing map here as early as possible after measurement deactivated it, so it can balance till next measurement is triggered
					TLE9012_DETECT_COM_ERROR( tle9012_writeRegisterByIndex(1, BAL_SETTINGS, statusBMS.bal_cell_map, 1, 0), tle9012_writeRegisterByIndex_BAL_SETTINGS , SUCCESS_FUNCTION_NONE);

					// Trigger multi-read of necessary registers
					tle9012_result = tle9012_readRegisterByIndex(1, MULTI_READ, 0, 0);
					if(tle9012_result >= REQUEST_ERROR){ PRINTF_MAIN_DEBUG("[ERROR]: Trigger next multi read returned %d\r\n", tle9012_result);}
				...
				// If the start bits are not gone yet trigger another read
					...
					PRINTF_MEASUREMENT("Measurement not finished - Queue measurement control again\r\n");

					// Read measurement control register again till start bit is gone
					tle9012_result = tle9012_readRegisterByIndex(1, MEAS_CTRL, 0, 0);
					if(tle9012_result >= REQUEST_ERROR){ PRINTF_MAIN_DEBUG("[ERROR]: Read MEAS_CTRL returned %d\r\n", tle9012_result);}
				}
			}
			return;
		...
		case MEASUREMENT_READ_RESULT:
			...
			if(checkRegisterQueued_tle9012(1, MULTI_READ, 0) == 0){
				...
				// BVM - Voltage of pack and write the result to statusBMS if successful
				TLE9012_DETECT_COM_ERROR( tle9012_convertBlockVoltage(1, 1)                 , tle9012_convertBlockVoltage , statusBMS.voltage = (float)tle9012_nodes[0].block_voltage);

				// PCVM - Primary cell voltage = voltage of each cell
				TLE9012_DETECT_COM_ERROR( tle9012_convertPrimaryCellVoltages(1, 0, 11, 1)   , tle9012_convertPrimaryCellVoltages          , SUCCESS_FUNCTION_NONE);

				// External Temperature
				TLE9012_DETECT_COM_ERROR( tle9012_convertTemperatures(1, 4, 1)              , tle9012_convertTemperatures                 , SUCCESS_FUNCTION_NONE);

				// Internal Temperature. NOTE: Only the first internal temperature sensor can be read with multi-read. Therefore the second one is ignored.
				TLE9012_DETECT_COM_ERROR( tle9012_convertInternalTemperatureByIndex(1, 0, 1), tle9012_convertInternalTemperatureByIndex_0 , SUCCESS_FUNCTION_NONE);
				//TLE9012_DETECT_COM_ERROR( tle9012_convertInternalTemperatureByIndex(1, 1, 1), tle9012_convertInternalTemperatureByIndex_1 , SUCCESS_FUNCTION_NONE);


				// Calculate the average temperature between cell temperature sensors
				float temp = (tle9012_nodes[0].temperatures[0] + tle9012_nodes[0].temperatures[3])/2;
				...
				statusBMS.avgCellVolt_mV = (uint16_t)((statusBMS.voltage/(float)NUM_CELLS)*1000);
		...
		case MEASUREMENT_TRIGGER_NEXT:
			...
			// Wait till the next measurement interval is reached. Note that there is a different interval while charging because the trigger is deactivating the balancing. Therefore the effectiveness of the balancing is improved by measuring slower while charging.
			if( (statusBMS.bms_state == BMS_CHARGING && (TIMER_COUNTER_1MS > (lastTLE9012MeasurementTimestamp + TLE9012_MEASURE_CHARGE_TIME))) ||
				(statusBMS.bms_state != BMS_CHARGING && (TIMER_COUNTER_1MS > (lastTLE9012MeasurementTimestamp + TLE9012_MEASURE_TIME))) ){
				...
				// Mark that the result of the measurement can be used this cycle. This will trigger e.g. manage of balancing
				isTLE9012ResultReceivedThisCycle = 1;

				// Trigger next measurement and set
				TLE9012_DETECT_COM_ERROR( tle9012_writeRegisterByIndex(1, MEAS_CTRL, (CVM_DEL__1*2) | PBOFF__BAL_INTERRUPT | BVM_MODE__16_BIT | BVM_START__TRIG_MEAS | CVM_MODE__16_BIT | PCVM_START__TRIG_MEAS, 1, 0), MEAS_CTRL_trigger_measurement , SUCCESS_FUNCTION_NONE);

				if(TriggerCurrentMeasurement == 1) PRINTF_MAIN_DEBUG("[ERROR]: processMeasurements tried to trigger next AF before ADC result was ready\r\n");

				// Mark that the analog frontend measurement was triggered and trigger measurement of current in this cycle
			...
			break;
		default:
			PRINTF_MAIN_DEBUG("[ERROR]: processMeasurements reached invalid state\r\n");
			statusBMS.measurementState = MEASUREMENT_TRIGGER_NEXT;
			break;
	}


	// Read external ADC and trigger next conversion -> Accurate current and output/load voltage measurement (current measurement will trigger in the same cycle as analog frontend TLE9012)
	manage_external_ADC_MCP3465(1);

	// Read internal ADC and trigger next conversion -> Rough driver current and slot coding resistor voltage measurement
	manage_internal_ADC();
}

//****************************************************************************
// In no error condition determine BMS_STATE charge direction and try to keep switch on (with precharge and minimum restart timer).
// In error condition keep switch off. Also controls orange LED (blink during precharge, on when conducting, off when error) and statusBMS.outputState
//****************************************************************************
void manage_SystemAndSwitchState(){
	uint8_t targetData[6];
	targetData[0] = 0;
	...
	// Get switch state
	uint8_t isSwitch_mainpath_ON  = getSwitchState_mainpath(USE_REG_RENEW_SS);   //uint8_t isSwitch_mainpath_ON = getStateChannelA(USE_REG_RENEW);
	uint8_t isSwitch_precharge_ON = getSwitchState_precharge(USE_REG_CACHED_SS); //uint8_t isSwitch_precharge_ON = getStateChannelB(USE_REG_CACHED);
	...
	if(isSwitch_mainpath_ON){ // Main (low resistance) path
		...
		// Blink Orange LED
		if(TIMER_COUNTER_1MS > (switchPrechargeStartTimestamp + blinkTimeLedOrange)){
			cyhal_gpio_toggle(BMS_LED2_OR);
	...
	// If BMS is plugged in and in a operational state, evaluate BMS state and manage switch, else keep switch off
	if((statusBMS.bms_state == BMS_CHARGING || statusBMS.bms_state == BMS_DISCHARGING || statusBMS.bms_state == BMS_IDLE) && (statusBMS.slotState != SLOT_NONE)){

		// Check if BMS is charging or discharging
		if((statusBMS.slotState == SLOT_IN_SYSTEM_1 || statusBMS.slotState == SLOT_IN_SYSTEM_2) && statusBMS.bms_state != BMS_DISCHARGING){ //(statusBMS.current      >  IDLE_CURRENT_THRESHOLD)
			statusBMS.bms_state = BMS_DISCHARGING;
		}
		else if(statusBMS.slotState == SLOT_IN_CHARGER && statusBMS.bms_state != BMS_CHARGING){ //(statusBMS.current < -IDLE_CURRENT_THRESHOLD)
			statusBMS.bms_state = BMS_CHARGING;
		}
		// BMS_IDLE state is only used for transition to discharge or charge and must therefore never be set here

		// If shutdown is triggered and conditions are given initiate HotSwap
		if(isShutdownTriggered == 1 && statusBMS.hotSwapRole == HOTSWAP_MAIN && (isSwitch_mainpath_ON & isSwitch_precharge_ON) == 1){
		...
		else if(isShutdownTriggered == 1 && (statusBMS.hotSwapRole == HOTSWAP_TAKER || statusBMS.hotSwapRole == HOTSWAP_ERROR)){
			// Shut down normally, no hotswap needed
			PRINTF_MAIN_DEBUG("Direct shutdown - BMS is in HotSwap TAKER or ERROR\r\n");
			shutdownSystem();
		}
		else if( (getMsgBit(MSG_INT_EXT_LOAD_V_ON) == 0) && (statusBMS.hotSwapRole == HOTSWAP_TAKER) && TIMER_COUNTER_1MS > HOTSWAP_TAKER_SHUTDWN_VEXT_TIME){
			// If BMS is hotswap TAKER and the external voltage disappears, the BMS can never taker over -> shutdown
			PRINTF_MAIN_DEBUG("Direct shutdown - BMS is in HotSwap TAKER and external voltage disappeared - could never take over\r\n");
			shutdownSystem();
		}
		...
		if(statusBMS.hotSwapRole == HOTSWAP_MAIN){
			switch (statusBMS.switchState){
				case SWITCH_IDLE:
					// If one of the safety switch channels are not conducting and time since last turn on is greater than minimal ...
					if((isSwitch_mainpath_ON & isSwitch_precharge_ON) == 0 && ( (TIMER_COUNTER_1MS > (lastSwitchOnTimestamp + MOSFET_ON_RETRY_TIME)) || lastSwitchOnTimestamp == 0) && (getMsgBit(MSG_INT_EXT_LOAD_V_ON) == 0 || statusBMS.slotState == SLOT_IN_CHARGER)){
						...
						isCurrentOffsetCalibrating = 0;
						setSwitchState_precharge(1); //switchCH_B_On(USE_REG_RENEW);
				...
				case SWITCH_PRECHARGE:
					...
					if(TIMER_COUNTER_1MS > (switchPrechargeStartTimestamp + PRECHARGE_MIN_TIME)){
						if( (TIMER_COUNTER_1MS > (switchPrechargeStartTimestamp + PRECHARGE_MAX_TIME)) || (statusBMS.voltageChangeRate_ms < PRECHARGE_TARGET_V_CHANGE_RATE) ){
							if(TIMER_COUNTER_1MS > (switchPrechargeStartTimestamp + PRECHARGE_MAX_TIME)) { PRINTF_MAIN_DEBUG("Switch on CH_A - Main path (maximum precharge time exceeded after %ldms)\r\n", TIMER_COUNTER_1MS-switchPrechargeStartTimestamp);}
							if(statusBMS.voltageChangeRate_ms < PRECHARGE_TARGET_V_CHANGE_RATE)          { PRINTF_MAIN_DEBUG("Switch on CH_A - Main path (target voltage change rate reached after %ldms - min precharge time %dms)\r\n", TIMER_COUNTER_1MS-switchPrechargeStartTimestamp, PRECHARGE_MIN_TIME);}

							// Turn main path on
							isCurrentOffsetCalibrating = 0;
							setSwitchState_mainpath(1); //switchCH_A_On(USE_REG_RENEW);
		...
		//// ---- GIVER ----
			...
			switch (statusBMS.giverHotSwapState) {
				case GIVER_SEND_HANDSHAKE:
					PRINTF_HOT_SWAP("GIVER_SEND_HANDSHAKE - Send BMS_HS_Handshake (A%d B%d)\r\n", getSwitchState_mainpath(USE_REG_RENEW_SS), getSwitchState_precharge(USE_REG_RENEW_SS));
				...
				case GIVER_WAIT_FOR_HANDSHAKE:
					...
					if(receivedCanHandshakeCount >= 1){
						PRINTF_HOT_SWAP("GIVER_WAIT_FOR_HANDSHAKE - Received, Send BMS_ReducePowerForHS! (A%d B%d)\r\n", getSwitchState_mainpath(USE_REG_RENEW_SS), getSwitchState_precharge(USE_REG_RENEW_SS));

						// Goto next state and reset timestamp
						statusBMS.giverHotSwapState = GIVER_WAIT_REDUCE_POWER;
						giverHotSwapTimestamp = TIMER_COUNTER_1MS;
					}
					else if(TIMER_COUNTER_1MS > (giverHotSwapTimestamp + HOTSWAP_TIMEOUT_TIME)){
						PRINTF_HOT_SWAP("GIVER_WAIT_FOR_HANDSHAKE - Timeout (A%d B%d)\r\n", getSwitchState_mainpath(USE_REG_RENEW_SS), getSwitchState_precharge(USE_REG_RENEW_SS));
						shutdownSystem();
					}
				...
				case GIVER_WAIT_REDUCE_POWER:
					...
					if(TIMER_COUNTER_1MS > (giverHotSwapTimestamp + HOTSWAP_WAIT_REDUCE_POWER_TIME)){
						PRINTF_HOT_SWAP("GIVER_WAIT_REDUCE_POWER - Send BMS_HS_Swap! (A%d B%d)\r\n", getSwitchState_mainpath(USE_REG_RENEW_SS), getSwitchState_precharge(USE_REG_RENEW_SS));
				...
				case GIVER_WAIT_SWAP:
					...
					if(receivedCanSwapCount >= 1){
						PRINTF_HOT_SWAP("GIVER_WAIT_SWAP - Received, output off and send BMS_HS_Finish! (A%d B%d)\r\n", getSwitchState_mainpath(USE_REG_RENEW_SS), getSwitchState_precharge(USE_REG_RENEW_SS));
						...
						// Shutdown system
						PRINTF_HOT_SWAP("GIVER_WAIT_SWAP - Finished, shutdown! (A%d B%d)\r\n", getSwitchState_mainpath(USE_REG_RENEW_SS), getSwitchState_precharge(USE_REG_RENEW_SS));
						shutdownSystem();
					}
					else if(TIMER_COUNTER_1MS > (giverHotSwapTimestamp + HOTSWAP_TIMEOUT_TIME)){
						PRINTF_HOT_SWAP("GIVER_WAIT_SWAP - Timeout (A%d B%d)\r\n", getSwitchState_mainpath(USE_REG_RENEW_SS), getSwitchState_precharge(USE_REG_RENEW_SS));
						shutdownSystem();
					}
					break;
			}
		...
		//// ---- TAKER ----
			...
			switch (statusBMS.takerHotSwapState) {
				case TAKER_WAIT_FOR_HANDSHAKE:
					...
					if(receivedCanHandshakeCount >= 1){
						PRINTF_HOT_SWAP("TAKER_WAIT_FOR_HANDSHAKE - Received, Send BMS_HS_Handshake! (A%d B%d)\r\n", getSwitchState_mainpath(USE_REG_RENEW_SS), getSwitchState_precharge(USE_REG_RENEW_SS));
				...
				case TAKER_WAIT_SWAP:
					...
					// If swap received turn on precharge and send command swap back
					if(receivedCanSwapCount >= 1){
						setSwitchState_precharge(1);
						PRINTF_HOT_SWAP("TAKER_WAIT_SWAP - Received, turn on precharge and send BMS_HS_Swap! (A%d B%d)\r\n", getSwitchState_mainpath(USE_REG_RENEW_SS), getSwitchState_precharge(USE_REG_RENEW_SS));

						// Goto next state and reset timestamp
						statusBMS.takerHotSwapState = TAKER_WAIT_FINISH;
						takerHotSwapTimestamp = TIMER_COUNTER_1MS;
					}
					else if(TIMER_COUNTER_1MS > (takerHotSwapTimestamp + HOTSWAP_TIMEOUT_TIME + HOTSWAP_WAIT_REDUCE_POWER_TIME)){
						PRINTF_HOT_SWAP("TAKER_WAIT_SWAP - Timeout  %ld %ld %d\r\n", TIMER_COUNTER_1MS, takerHotSwapTimestamp, HOTSWAP_TIMEOUT_TIME);
						shutdownSystem();
					}
				...
				case TAKER_WAIT_FINISH:
					...
					if(receivedCanFinishCount >= 1){
						PRINTF_HOT_SWAP("TAKER_WAIT_FINISH - Received, turn on full and goto MAIN mode! (A%d B%d)\r\n", getSwitchState_mainpath(USE_REG_RENEW_SS), getSwitchState_precharge(USE_REG_RENEW_SS));
						setSwitchState_mainpath(1); //switchCH_A_On(USE_REG_RENEW);
					...
					else if(TIMER_COUNTER_1MS > (takerHotSwapTimestamp + HOTSWAP_TIMEOUT_TIME)){
						PRINTF_HOT_SWAP("TAKER_WAIT_FINISH - Timeout \r\n");
						setSwitchState_precharge(0); //switchCH_B_Off(USE_REG_RENEW);
						shutdownSystem();
					}
	...
	// Output off condition
	else if(((isSwitch_mainpath_ON | isSwitch_precharge_ON) == 1) || (statusBMS.bms_state != BMS_ERROR && statusBMS.bms_state != BMS_IDLE)){
		...
		// Reset state machine switch to check and try switch on when error clears
		statusBMS.switchState = SWITCH_IDLE;

		// Change state to idle if needed
		if(statusBMS.slotState == SLOT_NONE && statusBMS.bms_state != BMS_IDLE && statusBMS.bms_state != BMS_ERROR){
			PRINTF_MAIN_DEBUG("BMS_IDLE state set (was %d)\r\n", statusBMS.bms_state);
			statusBMS.bms_state = BMS_IDLE;
		}
...
//****************************************************************************
// manage_analogFrontendTLE9012 - Execute queue management and interpretation of received data from anlog frontend. This also keeps the IC awake.
//                 See comment of tle9012_manageQueue and tle9012_manageInterpreter
//****************************************************************************
void manage_analogFrontendTLE9012(){
	// If init was not successful always report fail to signal error (and make system never switch on)
...
//****************************************************************************
// manage_external_ADC_MCP3465 - Read and trigger measurement of external adc subsequently for both channels
// returns 0 if measurement was not ready, 1 if current was read and 2 if external voltage was read
//****************************************************************************
uint8_t manage_external_ADC_MCP3465(uint8_t waitForCurrentMeasTrigger){
	uint8_t STATUS_DATA;
    ...
    // While waiting for the current measurement trigger do nothing. When it is set trigger the next measurement
	if(activeChannelExternalADC == ACTIVE_EXT_ADC_WAIT_FOR_CURRENT_TRIGGER){
		if(TriggerCurrentMeasurement == 1){
			...
			//PRINTF_MAIN_DEBUG("\tCurrent measurement triggered by TLE9012\r\n");
	...
	if (STATUS_DATA == 0x13) {
    	// Mark time of last successful read. Used to detect timeout error if communication fails for to long (diagnostic.c)
    	//PRINTF_MAIN_DEBUG("\tExternal ADC CH %d received after %lu ms\r\n", activeChannelExternalADC, TIMER_COUNTER_1MS-adcExtLastReadSuccessTimestamp); // Use this for ADC sample time measurement
        ...
        // Depending on which channel is currently active store result for it and trigger the other one
        if (activeChannelExternalADC == ACTIVE_EXT_ADC_CH0_CURRENT) {
        	...
        	float DATA_LOAD_mV = (float) avgFilter_I.filterResult * ADC_I_RAW_CONVERSION;		//	3300 / 65536

        	// Convert voltage with calibration to current
			float DATA_LOAD_CALIB_mA = (ADC_I_P1 * (float) DATA_LOAD_mV) - ADC_I_CONST;
        	//PRINTF_MAIN_DEBUG("\tMUX_CFG_CH0_SINGLE_END:   %.2fmV   ...   %X   ...   %.2fmA\r\n", DATA_LOAD_mV, SGN_DATA, DATA_LOAD_CALIB_mA);
        	...
        	if(isCurrentOffsetCalibrating == 0){
        		...
        		statusBMS.current = DATA_LOAD_CALIB_mA - (float)setupBMS.MeasCalib_Offset_I;
        	}
        	else{
        		// If system is calibrating do not use subtract but calculate calibrated offset
        		statusBMS.current = DATA_LOAD_CALIB_mA;
        		setupBMS.MeasCalib_Offset_I = (int16_t)(statusBMS.current - SYSTEM_DEFAULT_CURRENT_mA);
        		// Write debug message during calibration
        		PRINTF_MAIN_DEBUG("\tUpdate MeasCalib_Offset_I to %4dmA, with current %4.2fmA\r\n", setupBMS.MeasCalib_Offset_I, statusBMS.current);
        	}
			...
			// Convert raw value to voltage
    		float DATA_LOAD_mV = (float) system_V_raw * ADC_V_RAW_CONVERSION;		// ((18,2k + 1,2k) / 1,2k) 			//	(3300 / 65536)

    		// Convert raw voltage with calibration to current
    		float DATA_LOAD_CALIB_mV = (ADC_V_EXT_P1 * (float) DATA_LOAD_mV) + ADC_V_EXT_CONST;
    		//PRINTF_MAIN_DEBUG("\tMUX_CFG_CH1_SINGLE_END:   %X   ...   %X   ...   %.2fmV\r\n", DATA, SGN_DATA, DATA_LOAD_CALIB_mV);
        	...
        	calc_voltageChangeRate(statusBMS.extLoadVoltage, ADC_V_MIN_PLAUSIBLE);

        	// Wait for analog frontend to trigger current measurement
        	if(waitForCurrentMeasTrigger == 1)
        		activeChannelExternalADC = ACTIVE_EXT_ADC_WAIT_FOR_CURRENT_TRIGGER;
        	else{
        		// Trigger ADC Conversion on channel 0 current
        		SPI_WRT(_MUX_, MUX_CFG_CH0_SINGLE_END);
        		activeChannelExternalADC = ACTIVE_EXT_ADC_CH0_CURRENT;
...
//****************************************************************************
// manage_internal_ADC - Read and trigger measurement of internal adc for both channels
//****************************************************************************
void manage_internal_ADC(){
	...
	if (chanMask == (chanResultUpdated & chanMask))
	{
		...
		adc_safety_switch_current_mV = Cy_SAR_CountsTo_mVolts(SAR_HW, 1UL, adc_safety_switch_current);
...
void manage_externalCommunicationBMS(){
	...
	if(TIMER_COUNTER_1MS > (lastExternalCommunicationTimestamp + EXTERNAL_COMMUNICATION_TIME)){
		...
		//float safety_switch_current_mA = polyEvalLinear((float)adc_safety_switch_current_mV);

		PRINTF_CYCLIC_DEBUG_MSGS("V %5.2f, I %4.0fmA, VExt %5.2f, VImb %4d, BAL_MAP %4d, TH %5d, C %.1fmAh, C_SoH_i %.0fmAh, T_RR %9.3fmin, T_RT %5lums, T_E %3.1f %3.1f %3.1f, T_C %3.1f, T_I %3.1f, RTL %4u, CAN_ERR %3d, ",
				statusBMS.voltage,
				statusBMS.current,
				statusBMS.extLoadVoltage,
				statusBMS.imbalance,
		...
		//PRINTF_MEMORY_RT_DATA_UPDATE_RATE("\tRefresh rate %ld ms (dynamic %.0lfms/%ldlines remaining)\r\n", memoryRtDataRefreshTime, (float)batteryState.remaining_runtime_ms, (uint32_t)((MEM_ADDR_RT_DATA_STOP-MEM_ADDR_RT_DATA_START)/MEM_SIZE_RT_DATA)-(nextMemoryRtLineAddressOffset/MEM_SIZE_RT_DATA));
		...
		if(statusBMS.bms_state == BMS_CHARGING)
			...
			PRINTF_CYCLIC_DEBUG_MSGS("BMS STATE %d   , ", statusBMS.bms_state);
		...
		if(statusBMS.hotSwapRole == HOTSWAP_MAIN)
			...
			PRINTF_CYCLIC_DEBUG_MSGS("ROLE %d    , ", statusBMS.hotSwapRole);
		...
		if(latched_error_map_ext_com & ERR_CAF_OL_ERR){
			...
			printBinary((uint32_t)statusBMS.ol_map, 0b0000111111111111, 16);
			PRINTF_CYCLIC_DEBUG_MSGS(", ");
		}
		if(latched_error_map_ext_com & ERR_CAF_CELL_UV){
			PRINTF_CYCLIC_DEBUG_MSGS("UV_MAP ");
			printBinary((uint32_t)statusBMS.uv_map, 0b0000111111111111, 16);
			PRINTF_CYCLIC_DEBUG_MSGS(", ");
		}
		if(latched_error_map_ext_com & ERR_CAF_CELL_OV){
			PRINTF_CYCLIC_DEBUG_MSGS("OV_MAP ");
			printBinary((uint32_t)statusBMS.ov_map, 0b0000111111111111, 16);
			PRINTF_CYCLIC_DEBUG_MSGS(", ");
		}
		if(latched_error_map_ext_com & ERR_CAF_BAL_UC){
			PRINTF_CYCLIC_DEBUG_MSGS("UC_MAP ");
			printBinary((uint32_t)statusBMS.bal_uc_map, 0b0000111111111111, 16);
			PRINTF_CYCLIC_DEBUG_MSGS(", ");
		}
		if(latched_error_map_ext_com & ERR_CAF_BAL_OC){
			PRINTF_CYCLIC_DEBUG_MSGS("OC_MAP ");
			printBinary((uint32_t)statusBMS.bal_oc_map, 0b0000111111111111, 16);
			PRINTF_CYCLIC_DEBUG_MSGS(", ");
		}

		// Print bitmaps
		//PRINTF_CYCLIC_DEBUG_MSGS(", BMS_ERROR 0b"); printBinary(latched_error_map_ext_com, UINT32_MAX, 32);
		diagnostic_printStatusFlags(latched_error_map_ext_com, latched_message_map, 0, 0);
	...
	if(statusBMS.hotSwapRole == HOTSWAP_MAIN && (TIMER_COUNTER_1MS > (canLastMessageTimestamp + CAN_MINIMAL_MESSAGE_TIME + (rand() % 4)))){
		...
		// There are 3 types of messages (switchOnVeto for HotSwap, CAN requests and repeated status message sending) which are handled with given priority.
		cy_en_canfd_status_t can_status = UINT8_MAX;
		if(TIMER_COUNTER_1MS > (canSwitchOnVetoRefreshTimestamp + CAN_SWICH_ON_VETO_REFRESH_TIME)){
			// Store current time as last execution time
			canSwitchOnVetoRefreshTimestamp = TIMER_COUNTER_1MS;

			// Send switch on veto with data 200 to signal other BMS that this one is active
			targetData[0] = 200;
			targetData[1] = 1;
			...
			if(isCanRequested_RT_Reset >= 1){
				...
				PRINTF_MAIN_RT_CAN_REQUEST("\t CAN RT data line reset requested\r\n");
			}
			else if(isCanRequested_RT_GetLine == 1){
				// At the beginning of a line read the hole line to buffer, to be transfered in parts
				if(CanRequested_LinesCurrentIndexPart == 0){
					// Get data line as byte array
					memoryGetRtData_byteArray(memoryRtDataLineBuffer, 1, CanRequested_LinesStartIndex+CanRequested_LinesCurrentIndex);
					//memoryGetRtData(NULL, 1, CanRequested_LinesStartIndex+CanRequested_LinesCurrentIndex, 1);
					PRINTF_MAIN_RT_CAN_REQUEST("CAN RT data get lines requested, sending: start %u, stop %u, current %3u \r\n", CanRequested_LinesStartIndex, CanRequested_LinesNumber, CanRequested_LinesCurrentIndex);
					PRINTF_MAIN_RT_GETLINE_DEBUG("\tRead line %d to buffer %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X \r\n", CanRequested_LinesStartIndex, memoryRtDataLineBuffer[0], memoryRtDataLineBuffer[1], memoryRtDataLineBuffer[2], memoryRtDataLineBuffer[3], memoryRtDataLineBuffer[4], memoryRtDataLineBuffer[5], memoryRtDataLineBuffer[6], memoryRtDataLineBuffer[7], memoryRtDataLineBuffer[8], memoryRtDataLineBuffer[9]);
				}

				// Determine length to be transmitted (the last part of the current line may not fill a whole CAN message)
				uint8_t dataLength = CAN_MAX_DATA_LENGTH;
				if((CanRequested_LinesCurrentIndexPart+1)*CAN_MAX_DATA_LENGTH > MEM_SIZE_RT_DATA){
					dataLength = MEM_SIZE_RT_DATA - CanRequested_LinesCurrentIndexPart*CAN_MAX_DATA_LENGTH;  // MEM_SIZE_RT_DATA-(CAN_MAX_DATA_LENGTH*CanRequested_LinesCurrentIndexPart);
				}
				...
				if(CanRequested_LinesCurrentIndexPart > MEM_SIZE_RT_DATA/CAN_MAX_DATA_LENGTH){
					CanRequested_LinesCurrentIndex++;
					CanRequested_LinesCurrentIndexPart = 0;
					PRINTF_MAIN_RT_GETLINE_DEBUG("\tProcessing next line %d \r\n", CanRequested_LinesCurrentIndex);

					// If the current line index is higher than max request is finished
					if(CanRequested_LinesCurrentIndex > MEM_SIZE_MAX_LINES){
						isCanRequested_RT_GetLine = 0;
						PRINTF_MAIN_RT_GETLINE_DEBUG("\tProcessed last possible line. GetLine CAN request stopped \r\n");
					}

					// If the current line index is after stop index the request is finished
					if(CanRequested_LinesCurrentIndex > CanRequested_LinesNumber-1){
						isCanRequested_RT_GetLine = 0;
						PRINTF_MAIN_RT_GETLINE_DEBUG("\tProcessed last requested line. GetLine CAN request finished \r\n");
					}
				}
			}
				...
				if(isCanRequested_RT_GetStoredLineNumber > 5)
					isCanRequested_RT_GetStoredLineNumber = 0;
				PRINTF_MAIN_RT_CAN_REQUEST("\t CAN RT data get stored line number requested\r\n");
			}
				...
				if(isCanRequested_RT_GetMaxLineNumber > 5)
					isCanRequested_RT_GetMaxLineNumber = 0;
				PRINTF_MAIN_RT_CAN_REQUEST("\t CAN RT data get max line number requested\r\n");
			}
				...
				if(CanRequested_SamplingTime == 0){
					...
					PRINTF_MAIN_RT_CAN_REQUEST("CAN RT data set sampling time: Enabled dynamic sampling time");
				}else{
					memoryRtDataRefresh_isTimeFixed = 1;
					memoryRtDataRefreshTime = CanRequested_SamplingTime;
					PRINTF_MAIN_RT_CAN_REQUEST("CAN RT data set sampling time: Enabled fixed sampling time of %lu", memoryRtDataRefreshTime);
				}
				...
				if(isCanRequested_RT_GetSamplingTime > 5)
					isCanRequested_RT_GetSamplingTime = 0;
				PRINTF_MAIN_RT_CAN_REQUEST("\t CAN RT data get sampling time\r\n");
			}
		}
		else if(TIMER_COUNTER_1MS > (canDataRefreshTimestamp + CAN_DATA_REFRESH_TIME)){
			// Store current time as last execution time
...
void manage_timer(){
	...
	if(Cy_TCPWM_Counter_GetCounter(TIMER_1MS_HW, TIMER_1MS_NUM) > TIMER_1MS_config.period-5000){
		// Reset counter
		...
		// Reset all time storages //TODO low priority, does only happen after days of continuous run

	}
}

//****************************************************************************
// manage_slotState - Check the coding ADC channel and determine what state system is in
//****************************************************************************
void manage_slotState(uint8_t ignoreDebounce){
	// Decide in which slot state the system is
	if(adc_coding_res_mV > CODING_RES_MAINBOARD_2_BELOW_mV){
		// Write debug message and determine CAN ID only on initial slot change back to none
		if(statusBMS.slotState != SLOT_NONE){
...
//****************************************************************************
// manage_shutdown - Check if shutdown button is pressed and kill system by disabling shutdown pin
//****************************************************************************
void manage_shutdown(){
	...
	else if(TIMER_COUNTER_1MS > (deepDischargeTimestamp + DEEP_DISCHARGE_PROTECTION_TIME)){
		isDeepDischargeProtectionTriggered = 1;
	}

	// If shutdown button is pressed or deep discharge protection is activated...
	if((isShutdownTriggered == 0) && (cyhal_gpio_read(BUTTON_SHUTDOWN) == 1 || isDeepDischargeProtectionTriggered == 1)){
		PRINTF_MAIN_DEBUG("Shutdown triggered - Button %d, System voltage = %.2f (deep discharge protection below %.2f)\r\n", cyhal_gpio_read(BUTTON_SHUTDOWN), statusBMS.voltage, (float)DEEP_DISCHARGE_PROTECTION_V);
		// Mark time when shutdown was triggered and activate software watchdog if it is not already running (will shut down hard if shutdown takes longer than MAXIMUM_SHUTDOWN_TIME)
		shutdownTriggeredTimestamp = TIMER_COUNTER_1MS;
		isWatchdogActive = 1;
		// If system is in a system slot, mark that shutdown is triggered. In this state the HOTSWAP will be initialized and lead to shutdown after give-over or error shutdown
		if((statusBMS.slotState == SLOT_IN_SYSTEM_1 || statusBMS.slotState == SLOT_IN_SYSTEM_2) && (statusBMS.bms_state == BMS_CHARGING || statusBMS.bms_state == BMS_DISCHARGING)){
			isShutdownTriggered = 1;
		}
		// In any other state shut down directly
		else{
			PRINTF_MAIN_DEBUG("System is not in a 'wait for shutdown' condition. Direct shutdown:\r\n");
			shutdownSystem();
		}
	}

	// Note: Hard shutdown after MAXIMUM_SHUTDOWN_TIME is performed in software watchdog timer_watchdog_interrupt_handler()
}

//****************************************************************************
// keepHardwareWatchdogAlive - Send a kick to the hardware watchdog. This must be called faster than every HARDWARE_WATCHDOG_TIMEOUT ms
//****************************************************************************
void keepHardwareWatchdogAlive(){
	// Keep hardware watchdog alive
	#if ENABLE_HARDWARE_WATCHDOG == 1
...
//****************************************************************************
// shutdownSystem - Deactivate Watchdog and shutdown system by disabling shutdown pin
//****************************************************************************
void shutdownSystem(){
	...
	// Keep hardware watchdog alive one last time. If system does not shut down HARDWARE_WATCHDOG_TIMEOUT ms after this the watchdog will reset the system
	keepHardwareWatchdogAlive();

	// Make display refresh for the last time and disable shutdown pin
	for(uint16_t i = 0; i < LAST_UPDATE_DISPLAY_NUM; i++){
		// Blink green LED
		if(TIMER_COUNTER_1MS > (shutdownBlinkTimestamp + BLINK_TIME_LED_GREEN_SHUTDOWN)){
			cyhal_gpio_toggle(BMS_LED1_GR);
			shutdownBlinkTimestamp = TIMER_COUNTER_1MS;
		}
		// Update display and send shutdown command so it will refresh to main page and stop refreshing.
		updatePublicDisplayData(PUBLIC_COMMAND_SHUTDOWN);

		// When the last update is done the CM0+ core will return the done command
	...
	while(1){
		...
		// NOTE: If following line is inactive the system will stay in in shutdown if the power on button is held continuously
		//       If it is enabled the system will reboot and potentially start up/ switch ON again
		//NVIC_SystemReset();
	}
}

//****************************************************************************
// reset_safetySwitchTurnOn - Reset switch-on counter and reset state to make switch on possible again
//****************************************************************************
void reset_safetySwitchTurnOn(){
...
//****************************************************************************
// reset_latchedBitmaps - Resets each bitmap which content needs to be accumulated until a certain event happens. Does not effect the main/actual versions of the maps
//****************************************************************************
void reset_latchedBitmaps(uint8_t reset_error_map_switch_on, uint8_t reset_error_map_ext_com){
	// Reset the map that latches errors between switch ON of the safety switch if needed
	if(reset_error_map_switch_on != 0){
		latched_error_map_switch_on = 0;
	}
	// Reset the map that latches errors between PC/CAN transmissions if needed
...
//****************************************************************************
// calc_voltageChangeRate - Calculate the load voltage change rate if conditions are right (external voltage larger than minimal and at least on good measurement done before calculation)
// Writes the current rate if switch is in precharge mode
//****************************************************************************
void calc_voltageChangeRate(float loadVoltage, float minLoadVoltage){
	// Calculate voltage change rate
	if(loadVoltage > minLoadVoltage){
		// If the last read was successful calculate the change rate
		if(lastExtVoltageMeasurement < (ADC_EXT_V_CHANGE_RATE_MAX-0.01)){ // Float equivalent of != 0
			// Calculate change rate
			statusBMS.voltageChangeRate_ms = (loadVoltage - lastExtVoltageMeasurement)/(float)(TIMER_COUNTER_1MS - adcLastVoltageChangeRateTimestamp);
			// Print change rate only during precharge
			if(statusBMS.switchState == SWITCH_PRECHARGE){
				PRINTF_MAIN_DEBUG("\tVoltage change rate %11.8fV/ms, %5.3f-%5.3f in %ldms\r\n", statusBMS.voltageChangeRate_ms, loadVoltage, lastExtVoltageMeasurement, TIMER_COUNTER_1MS - adcLastVoltageChangeRateTimestamp);
			}
		}
...
uint8_t updatePublicDisplayData(public_core_commands command){
	...
	if((command == PUBLIC_COMMAND_SHUTDOWN) || (TIMER_COUNTER_1MS > (updatePublicDisplayDataTimestamp + UPDATE_PUBLIC_DISPLAY_DATA_TIME))){
		/// Update public variables from shadow buffer if resource is not used at the moment (non blocking)
		if (Cy_IPC_Sema_Status(SEMA_DISPLAY_VAR)        != CY_IPC_SEMA_STATUS_LOCKED){
			if(Cy_IPC_Sema_Set(  SEMA_DISPLAY_VAR, false) == CY_IPC_SEMA_SUCCESS){
				...
				// Error map
				sh_pub_error_map = latched_error_map_switch_on | statusBMS.error_map;
				...
				// Cell Voltage and diagnostic (3 MSBs are cellDiagnosticStates, rest is voltage in mV)
				for(uint8_t i = 0; i < NUM_CELLS; i++){
					/// Store voltage value of cell in shadow register
					sh_pub_cells[i] = tle9012_nodes[0].cell_voltages[i];
					// If voltage is to high set it to max (this should never happen)
				...
				// Update variables that are not refreshed every cycle (every cycle the next on will be refreshed)
				...
				// Get refresh state to determine if last refresh was done. if not wait till it is. If so write new value
				sh_pub_command = PUB_RAM_ACCESS(uint8_t, PUB9_COMMAND);
				if((command == PUBLIC_COMMAND_SHUTDOWN) && (sh_pub_command != PUBLIC_COMMAND_DONE)){
					PUB_RAM_ACCESS(uint8_t, PUB9_COMMAND) = command;
				}
				while (Cy_IPC_Sema_Clear(SEMA_DISPLAY_VAR,  false) != CY_IPC_SEMA_SUCCESS){__WFE();}
...
//{
//    /* Check if End-Of-Scan trigger has occurred. If yes, set SAR_HW_isr_set flag to true  */
...
void debugButtonCheck(){
	...
	// If button was detected as pressed send message and increment counter (Block until semaphore is free)
	if(cyhal_gpio_read(CYBSP_SWITCH) == 1 && (btn_pressed == 0)){
		...
		//// Trigger watchdog to test kill of system. Use first while to test software watchdog. Use second while to test hardware watchdog
		//while(1){
		//	1;
		//}
		//while(1){
		...
		//isWatchdogActive = 0;
		//memoryGetRtData(NULL, nextMemoryRtLineAddressOffset/MEM_SIZE_RT_DATA, 0, 1);
		//isWatchdogActive = 1;

		//// Send a UART message (Attempt to lock the semaphore)
		//while (Cy_IPC_Sema_Set(  SEMA_DEBUG_MSG, false) != CY_IPC_SEMA_SUCCESS){__WFE();}
		...
		//while (Cy_IPC_Sema_Clear(SEMA_DEBUG_MSG, false) != CY_IPC_SEMA_SUCCESS){__WFE();}
...
void debugRecordCycleTime(){
	...
	cycleTimeArray[cycleTimeArrayIndex] = TIMER_COUNTER_1MS - lastCycleTimestamp;
